...
uses Graficador,globales,cadenas,SScreen,def_Banderas,JPEG;

var
    FEsperar:TFEsperar;
    nomGra:TnombresGraficos;
...
begin
  {$I-}
  assignFile(f,Ruta_Aplicacion+'grf\Mapa.pal');
  reset(f,1);
...
begin
  x:=((x div ancho_tile)-MID_TILES_W+scrollbarx.position) div 4;
  y:=((y div alto_tile)-MID_TILES_H+scrollbary.position) div 4;
  limitar(x,y);
end;

procedure TFCmundo.ScreenToTiles(var x,y:integer);//calcular posicion en el tablero
begin
  x:=(((x{+12}) div ancho_tile)-MID_TILES_W+scrollbarx.position);
  y:=(((y{+8}) div alto_tile)-MID_TILES_H+scrollbary.position);
  limitarExt(x,y);
end;
...
begin
  result:=CrearBackBuffer16Bits(ancho_tile,alto_tile*4);
...
begin
  ...
  OpenDialog.InitialDir:=Ruta_Aplicacion+'bin';
  SaveDialog.InitialDir:=Ruta_Aplicacion+'bin';
  ...
  begin
    show;
    update;
    InicializarDirectDraw(self.handle);
    InicializarEfectos(Ruta_Aplicacion+CrptGDD);
    PBar.StepBy(40);
    LB_progreso.caption:='Recuperando gráficos';
    LB_progreso.repaint;
    InicializarConstantesTablero(Ruta_Aplicacion+'bin\',@nomGra);
    grTab:=TcoleccionGraficosTablero.create;
    InicializarMonstruos(Ruta_Aplicacion+'bin\std.mon');
    InicializarColeccionObjetos(Ruta_Aplicacion+'bin\obj.b');
    PBar.StepBy(50);
    LB_progreso.caption:='Creando nuevo mapa';
    LB_progreso.repaint;
  end;
  ...
  imgMarca:=CrearBackBuffer16Bits(ancho_tile,alto_tile);
  ...
  Caption:=Caption+GetVersionCliente();
  fesperar.free;
end;
...
begin
  ...
  begin
    ...
    if (x>(Scrollbarx.position div 4)-BIG_TILES_W) and (x<(Scrollbarx.position div 4)+BIG_TILES_W) and
       (y>(Scrollbary.position div 4)-BIG_TILES_H) and (y<(Scrollbary.position div 4)+BIG_TILES_H) then
      DibujarMapa;
  end;
end;
...
var i,j,pi,pj,x,y,ter_i,y_mod9:integer;
  ...
  begin
    ...
    case ter of
      ...
      begin
        mos_j:=y_mod9;
        mos_i:=XRandom(6,semilla);
      end;
      29,31:
      begin
        mos_j:=y_mod9;
        mos_i:=(256+x+y*2) mod 6;
      end;
      else
      begin
  ...
  begin
    ...
    for i:=0 to 99999 do
      BltAlphaTile(rDestino,ROrigen,0);
...
//Gráficos
  ...
  for j:=0 to MAX_TILES_H do
    for i:=0 to MAX_TILES_W do
    begin
      x:=i+coord.x-MID_TILES_W;
      y:=j+coord.y-MID_TILES_H;
      y_mod9:=(Ritmo_Tablero+y+256) mod 9;
      px:=x and $FFFFFFFC;
      py:=y and $FFFFFFFC;
      pi:=x and $3;
      pj:=y and $3;
      semilla:= jenkinsHash(x, y);
//Tiles:
...
begin
  ...
  if codigoDelGrafico<=MAX_OBJETOS_GRAFICOS then
    ...
    begin
      if (posy+LIMIT_Y0>coord.y) and (posy+LIMIT_Y1<coord.y) and
      (posx+LIMIT_X0>coord.x) and (posx+LIMIT_X1<coord.x) then
        if GrTab.Grafico[codigoDelGrafico]<>nil then //Seguridad necesaria
        begin
          x:=(posx+GRAF_OFF_X-coord.x)*ancho_tile;
          y:=(posy-InfGra[codigoDelGrafico].aliny+GRAF_OFF_Y-coord.y)*alto_tile;
  ...
  begin
    ...
    begin
      ...
      begin
        //Sprites
        x:=(posx+MID_TILES_W-coord.x)*ancho_tile;
        y:=(posy+MID_TILES_H-coord.y)*alto_tile;
...
//Gráfico a colocar, en transparente
  ...
  if (c_zoom.checked) then RealizarZoom();
...
begin
  if C_zoom.checked and (ssLeft in shift) then exit;
...
type
  TComerciante_mapa_legacy0=record
    //Generales:
    Tipo:byte;
    posx,posy,
    MonstruoComerciante:byte;
    item:array[0..17] of Tartefacto;//Objetos
    inflacion:array[0..17] of byte;//0= 255 objetos, 255=0 objetos
...
var i,j,k,x,y,p_x,p_y,codigoDelGrafico:integer;
    ...
    ComercianteLegacy0:TComerciante_mapa_legacy0;
begin
  assignFile(f,nombre);
  fileMode:=0;
  reset(f,1);
...
//Nr_Graficos,Nr_Sensores,Nr_nidos,Nr_PNJ
  ...
  for i:=0 to Nr_comerciantes-1 do
  begin
    if (DatosMapa.VersionMapa>0) then
    begin
      blockread(f,Comerciante[i],SizeOf(Comerciante[i]));
    end
    else
    begin
      blockread(f,ComercianteLegacy0,sizeof(ComercianteLegacy0));
      with Comerciante[i] do
      begin
        Tipo:=ComercianteLegacy0.Tipo;
        posx:=ComercianteLegacy0.posx;
        posy:=ComercianteLegacy0.posy;
        MonstruoComerciante:=ComercianteLegacy0.MonstruoComerciante;
        for j:=0 to 17 do item[i]:=ComercianteLegacy0.item[i];
        for j:=18 to MAX_ARTEFACTOS do item[i]:=ObNulo;
        for j:=0 to 17 do inflacion[i]:=ComercianteLegacy0.inflacion[i];
        for j:=18 to MAX_ARTEFACTOS do inflacion[i]:=0;
      end;
    end;
...
{  for i:=0 to Nr_npc-1 do
  ...
  begin
    E_RetornoX.text:=inttostr(posx_Resucitar);
    E_RetornoY.text:=inttostr(posY_Resucitar);
    E_RetornoM.text:=inttostr(mapa_Resucitar);
...
begin
  ...
  begin
    ...
    VersionMapa:=VERSION_ARCHIVO_MAPA;
...
//    N_NPC:=Nr_NPC;
    ...
    case cbTipoClimaMapa.itemindex of
      ...
      5://MapaOscuroConLluvia
        BanderasMapa:=BanderasMapa or bmSonidosBosqueOscuro or bmMapaOscuro or bmSinBruma;
    end;
...
{  for i:=0 to Nr_npc-1 do
  ...
  with DatosMapaExt do
  begin
    posX_Resucitar:=validar(E_RetornoX,0,255,0);
    posY_Resucitar:=validar(E_RetornoY,0,255,0);
    mapa_Resucitar:=validar(E_RetornoM,0,255,0);
...
begin
  if (screen.Width-left)<MIN_WINDOW_WIDTH then left:=screen.Width-MIN_WINDOW_WIDTH;
  if (screen.height-top)<MIN_WINDOW_HEIGHT then top:=screen.height-MIN_WINDOW_HEIGHT;
  if width<MIN_WINDOW_WIDTH then width:=MIN_WINDOW_WIDTH;
  if height<MIN_WINDOW_HEIGHT then height:=MIN_WINDOW_HEIGHT;
end;
...
procedure TFCmundo.BtnFxClick(Sender: TObject);
var i,n:integer;
    t:TDateTime;
    FXAmbiental:TClimaAmbiental;
    FxNocturno:TFxNocturno;
begin
  FXAmbiental:=TClimaAmbiental(cbFx.itemindex);
  if FXAmbiental>CL_BRUMA then
  begin
    FxNocturno:=TFxNocturno(Integer(FXAmbiental)-1-integer(CL_BRUMA));
    FXAmbiental:=CL_NOCHE;
  end
  else
    FxNocturno:=FxNocheAzul;
  if cb_testE.Checked then n:=1999 else n:=0;
  DibujarMapa;
  t:=now;
  case FXAmbiental of
    CL_NORMAL:
      for i:=0 to n do
        AplicarFXAmbientalRayo();
    CL_LLUVIOSO:
      for i:=0 to n do
