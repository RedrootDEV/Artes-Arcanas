...
begin
  for j:=0 to 2 do
    for i:=0 to 9 do
    begin
      artefacto:=Archivo.Artefactos[CmbComercio.itemindex][i+j*10];
      id:=artefacto.id;
...
const filename='..\laa\bin\comercio.b';

Type
  TInventarioArtefactos_Legacy0=array[0..17] of Tartefacto;
  TArchivoComercios_Legacy0=record
    Artefactos:array[0..MAX_TIPOS_COMERCIO] of TInventarioArtefactos_Legacy0;
    CheckSum:integer;
  end;

var f:file of TArchivoComercios;
    f_legacy0:file of TArchivoComercios_Legacy0;
    archivo_legacy0:TArchivoComercios_Legacy0;
    i,j:integer;
    result:boolean;
begin
  ...
  if not result then
  begin
    assignFile(f_legacy0,'..\laa\bin\comercio.b');
    reset(f_legacy0);
    {$I-}
      read(f_legacy0,archivo_legacy0);
    {$I+}
    result:=IOResult=0;
    closeFile(f_legacy0);
    if result then showmessage('Archivo actualizado') else showmessage('Archivo dañado');

    archivo.Version:=0;
    for i:=0 to MAX_TIPOS_COMERCIO do
      with archivo do
      begin
        for j:=0 to 17 do Artefactos[i][j]:=archivo_legacy0.Artefactos[i][j];
        for j:=18 to MAX_ARTEFACTOS do Artefactos[i][j]:=ObNulo;
      end;
  end;
  PaintBox.repaint;
end;
...
begin
  posicionTempo:=x div 40+(y div 40)*10;
