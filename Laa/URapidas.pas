...
implementation
{$R URapidas.dfm}
uses Graphics,UMensajes,globales,cadenas,Objetos,Juego,MundoEspejo;
...
procedure TFRapidas.formKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case key of
...
begin
  case Nro_accion of
    ...
    93:result:='Revisar cadaver o bolsa';
    94:result:='Activar ira berserker';
...
begin
  ...
  case CodAccion of
    3:begin//protección divina
      ...
      Jform.MensajeAyuda:='No tienes "'+NomObj[224]+'"';
    end;
    4..7:begin
      for i:=0 to MAX_ARTEFACTOS do
    ...
    125:begin
      for i:=0 to MAX_ARTEFACTOS do
        with JugadorCl.artefacto[i] do
          if (id>=144) and (id<156) and
            (infobj[id].modificadorADC<>(infobj[id].costo shl 2)) then
          begin
            Jform.RealizarAccion(mbRight,uConsumible,i);
            exit;
          end;
      if (JugadorCl.comida>25) then
        Jform.MensajeAyuda:='No tienes comida cocida o bebidas normales'
      else
      begin
        // Con mucha hambre, comer carne cruda
        for i:=0 to MAX_ARTEFACTOS do
          if ((JugadorCl.artefacto[i].id>=144) and (JugadorCl.artefacto[i].id<156)) then
          begin
            Jform.RealizarAccion(mbRight,uConsumible,i);
            exit;
          end;
        // Con mucha hambre, tomar bebidas especiales
        for i:=0 to MAX_ARTEFACTOS do
          if ((JugadorCl.artefacto[i].id>=156) and (JugadorCl.artefacto[i].id<160)) then
          begin
            Jform.RealizarAccion(mbRight,uConsumible,i);
            exit;
          end;
        Jform.MensajeAyuda:='No tienes nada para comer o beber';
      end;
    end;
    126:begin
