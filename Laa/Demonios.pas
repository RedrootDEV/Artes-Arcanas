...
const
  ...
  MaxRefrescamientoX=29;//16;
  MaxRefrescamientoY=24;//17;
  MaxVisionX=MaxRefrescamientoX-1;
  MaxVisionY=MaxRefrescamientoY-1;
  MaxAlcanceX=MaxVisionX-2;
  MaxAlcanceY=MaxVisionY-5;
  MaxComida=105;

  MAX_HABILIDADES_SUMADAS=38;
  MAX_NIVEL_NEWBIE=6;
  ...
  PENA_DEFENSA_BERSERKER=15;
  PENA_HP_BERSERKER=10;
  ...
  MANA_ZOOMORFISMO=3;
  MAXIMA_DISTANCIA_COMERCIO=4;
  TIEMPO_ATENCION_COMERCIANTE_AL_COMPRADOR=250;
...
// Flags de terreno en mapa lógico
  ...
  mskTerreno_Cultivos=ft_TerrenoSuave;//no es terreno sólido ni está cubierto
...
//Sincronización para optimizar comunicación
  flYaConoceSuBaul=$01;
  flModoPKiller=$02;
  flRevisandoBolsa=$04;// Evita enviar información de una bolsa continuamente
...
//Banderas positivas
  MskBanderasPositivas=BnBerserker or BnInvisible or BnArmadura or
...
{Flags:
  ...
  periciasDenegadas:array [0..7] of longword=(
    ...
    hbEscribir or hbAlquimia or hbBerserker or hbZoomorfismo,//Bribón
    hbEscribir or hbAlquimia or hbHerreria or hbTallarGemas,//Montaraz
    hbEscribir or hbAlquimia or hbApunnalar or hbZoomorfismo,//Paladín
    0,//Bardo
    0);//Guerrero Mago
  //Razas de jugadores
  rzHumano=0;
...
//  PInformacionMonstruos=^TInformacionMonstruos;
  ...
  public
    ...
    procedure inicializarTimer(TipoTimer:TTimerDemonio;valor:integer);
    ...
    function distancia2(objetivo:TMonstruoS):integer; //cuadrado de la distancia
  end;
  ...
  public
    ...
    function BaulACadena():TCadena95;
    function InventarioACadena():TCadena95;
    ...
    function ExtraerDatosEnCadena:Tcadena255;
    ...
    function PerderMeditarDescansar:boolean;
...
var
  ...
  PosicionesInicialesDeAvatares:array[0..8,0..2] of byte;
...
var FRZ,CON,INT,SAB,DES:byte;
    ...
    begin
      case cd_raza of //sumados max 20;
        rzElfo:Inicializar(3,7,2,6,2);
        rzSemielfo:Inicializar(3,6,3,5,2);
        rzEnano:Inicializar(5,2,8,2,2);
        rzGnomo:Inicializar(2,5,3,6,2);
        rzOrco:Inicializar(7,2,6,2,2);
        rzDrow:Inicializar(3,6,2,7,2);
        else Inicializar(2,2,2,2,2);
      end;
    end;
    procedure Minimos_por_Categoria;//sumados 26
    begin
      case cd_categoria of
        ctPaladin:Minimo(8,4,4,2,8);
        ctGuerrero:Minimo(10,6,6,2,2);
        ctMago:Minimo(2,6,2,10,6);
        ctClerigo:Minimo(6,4,4,2,10);
        ctBardo:Minimo(4,10,2,8,2);
        ctGuerreroMago:Minimo(8,6,2,8,2);
        ctBribon:Minimo(8,10,4,2,2);
        ctMontaraz:Minimo(6,8,2,2,8);
      end;
    end;
...
procedure TMonstruoS.inicializarTimer(TipoTimer:TTimerDemonio;valor:integer);
begin
  inc(valor,fTimer[byte(TipoTimer)]);
  if valor>255 then valor:=255;
  fTimer[byte(TipoTimer)]:=valor;
end;
...
function TMonstruoS.distancia2(objetivo:TMonstruoS):integer;
var dx, dy:integer;
begin
  dx:= coordx - objetivo.coordx;
  dy:= coordy - objetivo.coordy;
  result:= dx*dx + dy*dy;
end;
...
begin
  ...
  begin
    bono:=(bono*3) shr 1;
    bonoSuperior:=0;
    if (INT>=18) then inc(bonoSuperior,INT-17);
    if (SAB>=18) then inc(bonoSuperior,SAB-17);
    inc(bono,(bonoSuperior*(nivel-24)));
  end;
...
begin
  ...
  if longbool(banderas and BnBerserker) then inc(DannoBase,12);
...
begin
  ...
  if longbool(banderas and BnBerserker) then dec(defensa_basico,PENA_DEFENSA_BERSERKER);
...
begin
  ...
  //Modificadores de agilidad para evasión
  ...
  if Usando[uArmadura].modificador>0 then
  begin
  ...
  if Usando[uCasco].modificador>0 then
  begin
    id:=Usando[uCasco].id;
...
begin
  ...
  if (Usando[uarmadura].id>=4) and (Usando[uarmadura].modificador>0) then
  begin
    artefacto:=Usando[uarmadura].id-56;
    // Equipables como el ariete
...
var i:integer;
  ...
  var i:integer;
  begin
    Usando[0]:=ObjetoArtefacto(16,40);//daga
    Usando[1]:=ObManoIzqVacia;//daga
    for i:=2 to 7 do
      Usando[i]:=ObNulo;
    Artefacto[0]:=ObjetoArtefacto(144,5);//comida
    Artefacto[1]:=ObjetoArtefacto(152,5);//agua
    Artefacto[2]:=ObjetoArtefacto(222,20);//vendas
    Artefacto[3]:=ObjetoArtefacto(ihAfilador,50);
    Artefacto[4]:=ObjetoArtefacto(orLenna,40);
    Artefacto[5]:=ObjetoArtefacto(orBebidaAntiVeneno,25);
    ...
    case codCategoria of
      ...
      begin
        ...
        Artefacto[6]:=ObjetoArtefacto(124,1);//Varita mágica
        Artefacto[7]:=ObjetoArtefacto(orBebidaMasMana,10);
      end;
      ctBribon,ctBardo,ctMontaraz:
      begin
        Usando[uArmadura]:=ObjetoArtefacto(56,25);//armadura basica
        Usando[uMunicion]:=ObjetoArtefacto(52,60);//proyectiles
        Artefacto[6]:=ObjetoArtefacto(44,63);//honda
        Artefacto[7]:=ObjetoArtefacto(52,60);//proyectiles
      end;
      ctClerigo:
      begin
        Usando[uArmadura]:=ObjetoArtefacto(72,25);// Hábito
        Artefacto[6]:=ObjetoArtefacto(34,40);//garrote
        Artefacto[7]:=ObjetoArtefacto(116,1);//Símbolo clerical
      end;
      ctGuerrero:
      begin
        Usando[1]:=ObjetoArtefacto(80,30);//rodela
        Usando[uArmadura]:=ObjetoArtefacto(64,10);//armadura basica
        Usando[uCasco]:=ObjetoArtefacto(88,30);//casco simple
        Artefacto[6]:=ObjetoArtefacto(18,30);//espada corta
        Artefacto[7]:=ObjetoArtefacto(orBebidaMasHP,10);
      end;
    end;
  end;
...
function TjugadorS.PerderMeditarDescansar:boolean;
const BanderasPerdidas = BnVendado or BnDescansar or BnMana;
begin
  result := (accion = aaMeditando) or (accion = aaDescansando);
  accion:= aaParado;
  if longbool(banderas and BanderasPerdidas) then
  begin
    banderas:= (banderas or BanderasPerdidas) xor BanderasPerdidas;
    result:= true;
  end;
end;
...
//  if hp=0 then codAnime:=moFantasma;//Evitar la mancha de sangre que se mueve.
  for i:=0 to MAX_INDICE_PARTY do CamaradasParty[i]:=ccVac;
...
var ObjTemp:Tartefacto;
  ...
  begin
    ...
    case PosObjD of
      ...
      begin
        if (InfObj[idObjOrig].pesoArma=paPesada) and (Usando[uArmaIzq].id>=4) then
          ...
          begin
            ...
            Usando[uArmaIzq]:=ObManoIzqOcupada;
          end
          else
          begin
      ...
      uArmaIzq:
      begin
        if (ClaseObjetoOrigen=14) or (InfObj[idObjOrig].pesoArma=paPesada) then//Objetos que nunca se usan en la mano izquierda
        begin
          result:=i_ElObjetoVaEnOtraMano;
          exit;
        end;
        ...
        begin
          ...
          Usando[uArmaDer]:=ObManoDerVacia;
        end;
      end;
      uBrazaletes:
  ...
  if (PosObjO>=8) and (PosObjO<=MAX_POSICIONES) and (PosObjD<=MAX_POSICIONES) then //Que su posicion sea válida
    ...
    begin
      ...
      begin
        ...
        begin
          ObjTemp:=Artefacto[PosObjO];
          if (ObjTemp.id < 4) then
            if PosObjD=0 then
              ObjTemp:=ObManoDerVacia
            else if PosObjD=1 then
              ObjTemp:=ObManoIzqVacia
            else
              ObjTemp:=ObNulo;
          if Usando[PosObjD].id < 4 then Usando[PosObjD]:=ObNulo;
          Artefacto[PosObjO]:=Usando[PosObjD];
          Usando[PosObjD]:=ObjTemp;
        end;
        CalcularModDefensa;
    ...
    else//Intercambio con inventario
    begin
      dec(PosObjO,8);
      dec(PosObjD,8);
...
begin
  ...
  if temp<(maxHp shr 2) then //curar hasta llegar a amarillo
    temp:=maxHp shr 2
  else if temp>maxHp then
    temp:=maxHp;
  hp:= temp;
end;

procedure TjugadorS.Resucitar;
var nuevoNivelExp,tempExp:integer;
begin
  nuevoNivelExp:=ExperienciaNecesaria(nivel);
  if (nivel<32) then
  begin
    tempExp:=experiencia+((nuevoNivelExp*nivel) shr 5);
    if tempExp<nuevoNivelExp then nuevoNivelExp:=tempExp;
  end;
  ...
  begin
    if maxhp>15 then hp:=15 else hp:=maxhp;
    comida:=15;
  end;
  mana:=0;
  accion:=aaParado;
...
function TJugadorS.InventarioACadena():TCadena95;
var i:integer;
begin
  result:='';
  for i:=0 to 7 do
...
function TJugadorS.BaulACadena():TCadena95;
var i:integer;
begin
  result:='';
  for i:=0 to MAX_ARTEFACTOS do
    ...
    begin
      ...
      begin
        ...
        case idObj of
          144..151:
          begin
            if(infobj[idObj].modificadorADC = (infobj[idObj].costo shl 2)) and
              (comida>25) then
              result:=i_EstaCrudo
            else if (comida>=100) then
              result:=i_SinHambre;
          end;
          152..155:if (comida>=100) then result:=i_SinSed;
          orBebidaAntiVeneno:if (comida>25) and ((banderas and bnEnvenenado)=0) then result:=i_NoEstasEnvenenado;
          160:if (hp>=maxHp) then result:=i_SinHeridas;//HP
          orBebidaMasHP:if (comida>25) and (hp>=maxHp) then result:=i_SinHeridas;//HP
          orBebidaMasManaHp:
            if (comida>25) then
            begin
              if (hp>=maxHp) then
                if (maxMana=0) then
                  result:=i_SinHeridas//HP
                else if (mana>=maxMana) then
                  result:=i_ManaMaximo;//Mana
            end;
          orBebidaMasMANA:
            if (comida>25) then
            begin
          ...
          162:
            if (maxMana=0) then
              ...
              if mana>=maxMana then result:=i_ManaMaximo;//Mana
          165:
            if not esNecesarioSanar then result:= i_NoDesperdiciasPocima;
        end;
      end;
    end
  else
...
begin
  ...
  if (Conjuros and (1 shl ConjuroElegido))<>0 then
    ...
    if ((nivelINT<=INT) and (nivelSAB<=SAB) and (nivelJugador<=nivel)) or ((nivelJugador shl 1)<=nivel) then
...
begin
  ...
  if RitmoDeVida<34 then RitmoDeVida:=34;
end;
...
function TjugadorS.PuedeActivarBerserker:byte;
begin
  if hp<>0 then
    if (banderas and bnBerserker)=0 then
      if longbool(pericias and hbBerserker) then
        if (hp>PENA_HP_BERSERKER) then
          result:=i_Ok
        else
          result:=i_IraNecesitaMasHP
      else
        result:=i_NoTienesLaPericiaBerserker
    else
      result:=i_error
  else
    result:=i_EstasMuerto
...
begin
  ...
  //si existe demasiadas pericias borrar todas y asignar posibles
  ...
  begin
    CON:=TotalPuntos shr 3;
    dec(TotalPuntos,CON);
    INT:=TotalPuntos shr 2;
    dec(TotalPuntos,INT);
    dec(TotalPuntos,FRZ);
    dec(TotalPuntos,DES);
    SAB:=TotalPuntos;
...
begin
  ...
  begin
    ...
    if LongBool(Pericias and hbElocuencia) then
    ...
    if (not EsPrecioVenta) and BonoClan then inflacion:=inflacion shr 1;
    if inflacion<0 then inflacion:=0;
    if inflacion>255 then inflacion:=255;
    result:=result*(inflacion+384);
    if EsPrecioVenta then result:=(result shr 10) else result:=(result shr 9);
...
function TjugadorS.ExtraerDatosEnCadena:Tcadena255;
begin
  ...
  //no temporales
    ...
    b4aStr(getHabilidades)+InventarioACadena+chr(ConjuroElegido)+
    nombreAvatar[0]+nombreAvatar;
end;
...
function TJugadorS.esNecesarioSanar():boolean;
begin
  result:=longbool(banderas and MskBanderasSanadas) or (hp<(maxhp shr 2));
end;

function TJugadorS.TiempoConjuro(EsConjuroArcano:boolean):byte;
begin
  result:=6+nivel shr 3;
