...
const
  ...
  //TERBOL
  maxBolsas=255; //hasta 1022 ( maximo 1022, 1023 está reservado para indicar que no existe bolsa)
  mskBolsa=$3FF;//el campo TerBol and mskBolsa da el codigo_bolsa
  mskTerreno=$FC00; // Flags que describen el terreno
  msk_terreno_tiles=$1F; // Codigo de grafico para un tile
  ...
  TIPO_COMERCIANTE_MERCADER=15;
  max_sensores=249;//Sensores por mapa

  max_guardianes=1;//2 guardianes: 0 y 1
...
// Otros
  ...
  fs_ConsumirLlave=$01;
  fs_RepelerAvatar=$02;
  // Flag con comportamiento distinto para otros tipos de sensores
  fs_AreaActivaGrande=fs_RepelerAvatar;
  fs_SoloClan=$10;
  fs_SoloAprendiz=$20;
  fs_SoloFantasma=$40;
...
//Inf. general del tablero
  ...
  TDatosMapa=record//No modificar
    ...
    VersionMapa,N_nidos,N_NPC,N_Comerciantes:byte;
...
//alinear a 4Bytes, nunca agregar campos ANTES de los que ya existen
    //Posicion para resucitar
    posx_Resucitar,posy_Resucitar:byte;
    mapa_Resucitar,noUsadoX2:byte;//2bytes, 4 en total
...
{  TNPC_mapa=record //Será objeto
  ...
  TComerciante_mapa=record
    ...
    // precio calculado en TjugadorS.CalcularModificacionPrecio
  end;
             ...
             terBol:word;//terreno y bolsas
{
  ...
  TMapaAreaPos=array[0..MaxMapaAreaExt,0..MaxMapaAreaExt] of TCasMapa;
  PMapaAreaPos=^TMapaAreaPos;

  TMapaAreaSensor=array[0..MaxMapaAreaExt,0..MaxMapaAreaExt] of byte;
...
//  TmapaAreaSimple=array[0..MaxMapaArea,0..MaxMapaArea] of byte;
  ...
  TMapaTiles=array[0..MaxMapaAreaExt, 0..MaxMapaAreaExt] of byte;
...
//SOLO ALCUNOS METODOS PARA EL CLIENTE Y PARA EL SERVIDOR
    ...
    function SePuedeMeditarDescansar(x,y:byte):boolean;
  end;
  ...
  function calcularDireccion(deltax,deltay:integer):TDireccionMonstruo;

  function PendienteDeClima(tipoClima:TClimaAmbiental):shortint;
...
// definidos por: InicializarColeccionPosicionesGraficos
  ...
  TablaCalcularDir:array[0..255] of word;
...
//  read(f,InfGraficos);
  ...
  // Inicializar tabla para calcular direccion
  TablaCalcularDir[0]:= 51200;
  for i:=1 to 255 do
  begin
    TablaCalcularDir[i]:= 51200 div i;
  end;
...
// Optimiza la función de encontrar criaturas enemigas.
  ...
  begin
    for j:=-1-i to 1+i do
    begin
      MC_DE_Direcciones[contador]:=calcularDireccion(j,-i-1);
    ...
    for j:=-i to i do
    begin
      MC_DE_Direcciones[contador]:=calcularDireccion(i+1,j);
...
CONST
  UMBRAL_MOVIMIENTO_EN_Y=83;
  UMBRAL_MOVIMIENTO_EN_X=483;
...
begin
  ...
  begin
    if (deltay<>0) then
    begin
      factor:=(abs(deltay) * TablaCalcularDir[abs(deltax) and $FF]) shr 8;
      if factor<=UMBRAL_MOVIMIENTO_EN_Y then
        deltay:=0
      else if factor>=UMBRAL_MOVIMIENTO_EN_X then
        deltax:=0;
    end;
    if deltaX<0 then deltaX:=-1
      else if deltaX>0 then deltaX:=1;
  end;
  if deltaY<0 then deltaY:=-1
    else if deltaY>0 then deltaY:=1;
...
begin
  if Bolsa<>nil then freeMem(Bolsa);
  freeMem(mapaSensor);
  freeMem(mapaPos);
  inherited destroy;
end;
...
var
  ...
  procedure DefinirMapaTiles();
  var
    i,j:integer;
    function getTerrenoXY(x,y:integer):integer;
    begin
      limitarExt(x,y);
      result:=MapaTiles[x,y] and msk_Terreno_tiles;
    end;
  ...
  begin
    ...
    // Definir los bits de paleta de colores para fluidos en $E000
    for j:=0 to MaxMapaAreaExt do
      for i:=0 to MaxMapaAreaExt do
      begin
        //MapaTiles[i,j]:=MapaTiles[i,j] or flagsPaleta;
      end;
  end;
...
{  for i:=0 to N_npc-1 do
  ...
  posX_Resucitar:=DatosMapaExt.posX_Resucitar;
  posY_Resucitar:=DatosMapaExt.posY_Resucitar;
  Mapa_Resucitar:=DatosMapaExt.mapa_Resucitar;
  ...
  {$I+}
  result:=ioresult=0;
  result:=result and (DatosMapa.VersionMapa=VERSION_ARCHIVO_MAPA);
  ...
  CrearMapaDeSensores;
  // Antes de crear el mapa lógico copiar los tiles a mapaTiles
  if MapaTiles<>nil then DefinirMapaTiles();
  CrearMapaLogico;
end;
...
begin
  ...
  hayFogata:=(recurso=irFundicion) or (recurso=irFogata);
  jug.aurasExternas:=jug.aurasExternas or flAuraExtFogata;
  if (not hayFogata) then
    ...
    begin
      ...
      if codigoBolsa<=maxBolsas then
        if (bolsa[codigoBolsa].tipo>=tbFogata) then
        begin
          hayFogata:= true;
          break;
        end;
  ...
  if (not hayFogata) then
    jug.aurasExternas:=jug.aurasExternas xor flAuraExtFogata;
end;
...
begin
  ...
  begin
    ...
    terreno:=mapaPos[x,y].terBol;
    if ((terreno and ft_agua)<>0) or
      ((terreno and mskTerreno)=mskTerreno_InteriorVivienda) or
      ((terreno and mskTerreno)=mskTerreno_Cultivos) then exit;
    // Verificar si existe una bolsa
    if (mapaPos[x,y].terbol and mskBolsa)<=maxBolsas then exit;
  end;
  result:=true;
end;
...
begin
  ...
  begin
    ...
    recurso:=mapaPos[x,y].monRec;
    if recurso>=ccRec then
    begin
      recurso:=recurso and $FF;
      if recurso<>$FF then
      begin
        result:=recurso;
        exit;
    ...
    //casilla vacia de recursos
    if (mapaPos[x,y].terbol and mskTerreno)=ft_Agua then
      result:=irAguaConPeces
    else
    begin
      recurso:=mapaPos[x,y].terbol and mskBolsa;
      if recurso<=maxBolsas then
        if (bolsa<>nil) then
        begin
          // server
          if (bolsa[recurso].tipo>=tbFogata) then result:=irFogata;
        end
        else
          //client
          if TTipoBolsa(recurso)>=tbFogata then result:=irFogata;
    end;
  end;
end;
  ...
  begin
    if recurso=irFundicion then
      if longbool(jug.Pericias and (hbMineria or hbHerreria)) then
        ...
        begin
          if (minimoMineral >= 16) then result:=i_NecesitasMasOro
          else if (minimoMineral >= 8) then result:=i_NecesitasMasPlata
          else result:=i_NecesitasMasMineral;
        end
      else
...
//Para verificar los tipos de objetos.
  ...
  if (hp>0) or (artefacto[indArt].id=orBaulMagico) then
    ...
    begin
      ...
      case idArticulo of
        ...
        ihVendas:
          if (hp>=maxhp) or ((banderas and bnVendado)<>0) then
        ...
        ihTallador:
          if recurso=irTalladoGemas then
        ...
        orPlata:result:=ControlFundicion(8);
        orOro:result:=ControlFundicion(16);
        ...
        begin
          ...
          begin
            if recurso=irSastreria then
              ...
              if Usando[uArmaDer].id<4 then
                result:=i_SinSastreria;
          end;
          if result=i_error then
          begin
            if InfObj[Usando[uArmaDer].id].TipoReparacion=trCoser then
              ...
              begin
                ...
                result:=i_NoPuedesRepararMejor
            else if InfObj[Usando[uArmaIzq].id].TipoReparacion=trCoser then
              result:=i_ObjetoReparableVaEnManoDer
            else if (InfObj[Usando[uArmaDer].id].TipoReparacion=trNoReparable) and
              (InfObj[Usando[uArmaDer].id].HerramientaRequerida=ihTijeras) then
              result:=i_ObjetoNoReparable
            else if longbool(Pericias and hbSastreria) then
              result:=i_NecesitasManoDerechaLibre
            else
          ...
          begin
            if InfObj[Usando[uArmaDer].id].TipoReparacion=trMartillar then
              ...
              begin
                ...
                result:=i_NoPuedesRepararMejor
            else if InfObj[Usando[uArmaIzq].id].TipoReparacion=trMartillar then
              result:=i_ObjetoReparableVaEnManoDer
            else if (InfObj[Usando[uArmaDer].id].TipoReparacion=trNoReparable) and
              ((InfObj[Usando[uArmaDer].id].HerramientaRequerida=ihMartillo) or
               (InfObj[Usando[uArmaDer].id].HerramientaRequerida=ihSerrucho)) then
              result:=i_ObjetoNoReparable
            else if longbool(Pericias and hbHerreria) then
              result:=i_NecesitasManoDerechaLibre
            else
        ...
        ihSerrucho:
          if longbool(Pericias and hbCarpinteria) then
            if recurso=irCarpinteria then
              if Usando[uArmaDer].id<4 then
                result:=i_OK
              else
                result:=i_NecesitasManoDerechaLibre
            else
              result:=i_SinCarpinteria
          else
        ...
        ihPergaminoA,ihPergaminoS:
          if (maxMana>0) then
            if PuedeLeerElConjuro(artefacto[indArt].modificador) then
              if 0=(Conjuros and (1 shl artefacto[indArt].modificador)) then
                if (mana>=maxMana) then
                  result:=i_Ok
                else
                  result:=i_NecesitasMaximoMana
              else
...
begin
  ...
  py:=y;
  repeat
    // Return ccVac if there is no monster in the main cell but there is
    // something else
    if (iPos=0) then
      if getCodBolsaVerificarFronterasXY(x,y)<>NoExisteBolsa then
      begin
        result:=ccVac;
        exit;      
      end;
...
function TTablero.modificadorClima(Infravision:boolean):integer;
//Negativo: menor Defensa, Positivo: mayor defensa
begin
  if longbool(BanderasMapa and bmMapaOscuro) or (TipoClimaGeneral=CL_NOCHE) then
    if Infravision then
      result:=5//Oscuridad, bono +5 en evasión (atacante con infravision)
    else
      result:=15//Oscuridad, bono +15 evasión
  else
    if (TipoClimaGeneral=CL_BRUMA) and not longbool(BanderasMapa and bmSinBruma) then
      result:=15//Bruma, bono +15 evasión
    else
      result:=0;//Normal.
end;

function TTablero.EstaLloviendo(x,y:byte):boolean;
begin
  result:=((TipoClimaGeneral=CL_LLUVIOSO) or (TipoClimaGeneral=CL_LLUVIA_NOCHE)) and
    (IntensidadClimaGeneral>40) and (not longbool(BanderasMapa and bmSinLluvia)) and
    not longbool(BanderasMapa and bmSonidosHielos) and
    ((MapaPos[x,y].terbol and ft_Cubierto)=0);
end;

function TTablero.SePuedeMeditarDescansar(x,y:byte):boolean;
begin
  result:=not (((TipoClimaGeneral=CL_LLUVIOSO) or (TipoClimaGeneral=CL_LLUVIA_NOCHE)) and
    (IntensidadClimaGeneral>40) and (not longbool(BanderasMapa and bmSinLluvia)) and
    ((MapaPos[x,y].terbol and ft_Cubierto)=0));
end;
