...
const
  ...
  MAX_SPRITES_FX2=199;//Número máximo de efectos en la pantalla, usa buffer circular.
  MAX_SPRITES_FX=63;
  BASE_SPRITES_FX2=MAX_SPRITES_FX + 1;
...
type
  ...
  private
    fTipoAnimacion,fCoord_fx_x,fcoord_fx_y:array[0..MAX_SPRITES_FX2] of byte;
    fdireccion:array[0..MAX_SPRITES_FX2] of TDireccionMonstruo;
    fFrame:array[0..MAX_SPRITES_FX2] of shortint;
    fMonstruo:array[0..MAX_SPRITES_FX2]of TmonstruoS;//nil= fx estático
    posLibre:integer;
    posLibre2:integer;
  public
    constructor Create;
  ...
  function AgregarSufijoNeutroEse(const nombre:string):string;
...
implementation
uses sonidos,Graficos,MundoEspejo,sysutils,cadenas,globales,juego;
...
begin
  ...
  if nivel_3125 and $7=0 then
    result:='+'+inttostr((nivel_3125 shr 3)*25)
  else
...
begin
  ...
  begin
    ...
    drawPoderMonstruo;
    if Mostrar_Nombres_Sprites and (not Graficos_Transparentes) then
...
begin
  refJugador:= nil;
  if refMonstruo is TJugadorS then
  begin
    refJugador:= TJugadorS(refMonstruo);
    canSee:= (refMonstruo.codAnime<Inicio_tipo_monstruos) or // Not disguised
      (refMonstruo.comportamiento>comHeroe) or // Game mater is always seen
      (JugadorCl.CapacidadId=ciVerRealmente) or // Has spell to see
      (JugadorCl.comportamiento>comHeroe); // Game master sees everything
  end
  else
    canSee:= refMonstruo.comportamiento=comComerciante;
  if (canSee) then drawNombre;
end;
...
begin
  ...
  case mensaje of
    ...
    i_TeFaltaNivelParaUsarElObjeto:begin
      icono:=Artefacto[nroArtefacto].id;//este es el id. del objeto
    ...
    i_LugarNoAdecuadoParaTrampa:result:='Este no es buen lugar para la trampa';
    i_LugarNoAdecuadoParaFogata:result:='Este no es buen lugar para la fogata';
    ...
    i_EstasMuyLejos:result:='Tienes que acercarte, estás muy lejos';
    i_NecesitasAmbasManos:result:='Tus manos están ocupadas y tu bolsa llena';
    i_ArmaDestrozada:result:='No puedes atacar con tu arma rota';
    ...
    i_NoDesperdiciasPocima:result:='No necesitar usar la pócima';
    ...
    i_NecesitasMasMineral:result:='Necesitas 4 minerales para un lingote';
    i_NecesitasMasPlata:result:='Necesitas 8 para un lingote de plata';
    i_NecesitasMasOro:result:='Necesitas 16 para un lingote de oro';
    i_NoSabesHacerPocimas:result:='Sólo un herbalista puede crear pócimas';
    i_SinSastreria:result:='Tienes que estar frente a una mesa de sastre';
    i_SinTelar:result:='Tienes que estar frente a un telar';
    i_ArruinasteLaGema:result:='Arruinaste la gema';
    i_noFabricasTela:result:='Sólo un sastre puede fabricar tela';
    i_noEresSastre:result:='No tienes la pericia Sastrería';
    i_noEresHerrero:result:='No tienes la pericia Herrería';
    i_noEresCarpintero:result:='No tienes la pericia Carpintería';
    i_noEresAlquimico:result:='Desconoces los secretos de la Alquimia';
    i_noEresMagoEscritor:result:='No tienes la pericia para escribir magia';
    i_NecesitasMasFibras:result:='Necesitas al menos 4 para fabricar tela';
    ...
    i_NecesitasMaximoMana:result:='Tu maná tiene que estar al máximo';
    ...
    i_SinArmaAceitable:result:='Coloca el arco o ballesta en tu mano derecha';
    i_ObjetoReparableVaEnManoDer:result:='Coloca el objeto a reparar en tu mano derecha';
    i_ObjetoNoReparable:result:='Ese objeto no es reparable';
    i_NoPuedesRepararMejor:
      if nivel>=MIN_NIVEL_CATEGORIA then
        result:='Está en buenas condiciones, no necesita reparación'
      else
        result:='No puedes repararlo mejor de lo que está';
    ...
    i_NoTienesLaPericiaBerserker:result:='No tienes la pericia Ira Berserker';
    i_IraNecesitaMasHP:result:='Ira Berserker necesita más de '+intastr(PENA_HP_BERSERKER)+' puntos de salud';
    i_NecesitasManoDerechaLibre:result:='Desocupa tu mano derecha para fabricar';
    ...
    i_ComandoAdministrativoEjecutado:result:='Comando administrativo ejecutado';
    ...
    i_NecesitasGema50a100:result:='Necesitas una Gema de 50% a 100% de calidad';
    ...
    i_SeleccionaObjetoInventario:result:='Elige un objeto del inventario';
    i_SeleccionaObjetoBolsa:result:='Elige un objeto de la bolsa';
    ...
    i_noEsEnvenenable:result:='El objeto en tu mano derecha no es envenenable';
    i_MagicaSobreMuerto:result:='Este hechizo no actúa en muertos';
    i_NoSabesCurtir:result:='Necesitas la pericia sastrería para curtir pieles';
    i_SinCurtidora:result:='Tienes que estar frente a una tina de curtido';
    i_NecesitasMasPieles:result:='Necesitas al menos 4 pieles';
    ...
    i_NoEresBardo:result:='Desafortunadamente no eres bardo';
    ...
    i_EstaCrudo:result:='Esta comida está cruda, hay que cocinarla';
    ...
    i_YaTieneClan:result:='Ese avatar ya tiene un clan';
    ...
    i_30monedasDePlata:result:='Pon 30 monedas de plata en tu mano derecha';
    ...
    i_SinCarpinteria:result:='Tienes que estar frente a un taller de carpintería';
    i_SinTallerJoyeria:result:='Tienes que estar frente a un taller de joyería';
    ...
    i_NoComproEsasCosas:case XRandom(3) of
      0:result:='No vendo ni compro ese tipo de objetos';
      1:result:='No trabajo con esa mercadería';
      else result:='Eso no lo compro, sólo un mercader compra de todo';
    end;
    i_NoIntentesEstafarme:result:='No es lo acordado, no hay trato';
    i_DialogoNPJqueCompro:case XRandom(3) of
      0:result:='Espero que sigamos haciendo negocios';
      1:result:='¿Algo más que quieras vender?';
      else result:='Talvez quieras comprar algo';
    end;
    i_DialogoNPJqueVendio:case XRandom(3) of
      1:result:='¿Deseas comprar algo más?';
      2:result:='Sigue comprando lo que necesites';
      else result:='Gracias y buena suerte';
    end;
...
{        ihPico:
        ...
        orFibras:result:='Fabricaste tela';//fabricar tela
        ihAfilador:
        begin
        ...
        ihAceite:result:='Das mantenimento a tu '+NomObj[Usando[uArmaDer].id];
        ihMartillo:
      ...
      if longbool(banderas and BnCongelado) then
        ...
        if longbool(banderas and bnEnvenenado) then
          ...
          if longbool(banderas and (bnParalisis or bnAturdir)) then
            ...
            if longbool(banderas and (BnBerserker or bnZoomorfismo)) then
...
begin
  ...
  begin
    case Artefacto[NroObjetoBaul].id of
      ...
      ihTallador:IdSonido:=snTallarGemas;
    ...
    SonidoIntensidad(IdSonido,-XRandom(128));
  end;
end;
...
begin
  if (EspecialidadArma and mskEspecialidadFija)<>0 then
    ...
    result:='Cambiando especialidad: ';
  result:=result+NomObj[EspecialidadArma and mskEspecialidadObjeto]+', +'+
    intastr((NivelEspecializacion shr 2)+1)+'% ataque';
  if (NivelEspecializacion>=4) then
  begin
    result:=result+', '+DannoPorcentual_3125((NivelEspecializacion+4) shr 3);
  end;
end;
...
begin
  if maxMana>0 then
    with InfConjuro[nroconjuro] do
      result:=NomConjuro[nroconjuro]+', nivel:'+intastr(nivelJugador)+
      ', maná:'+intastr(nivelMana)+', I:'+intastr(nivelINT*5)+', S:'+intastr(nivelSAB*5)
  else
...
begin
  ...
  if comportamiento>comHeroe then
    ...
    result:=result+InfMon[TipoMonstruo].nombre;
...
begin
  ...
  if (banderas and bnBerserker)<>0 then result:=result+' Berserker,';
  ...
  if (banderas and bnEfectoBardo)<>0 then result:=result+' Inspirado,';
    ...
    begin
      ...
      if (fmensajeImportante[posicion] and (fTimerChat[posicion]>0) and (posDibujo<=2))
        ...
        begin
          ...
          TextOut(4,OFFSET_Y - posDibujo*14,fmensaje[posicion]);
          inc(posDibujo);
        end;
    end;
  end;
...
procedure TControlfx.SetEfecto(x,y:byte;TipoAnimacion:byte;frame:shortint;direccion:TdireccionMonstruo;monstruo:TmonstruoS);
  procedure setData(var index:integer);
  begin
    fmonstruo[index]:=monstruo;
    fCoord_fx_x[index]:=x;
    fCoord_fx_y[index]:=y;
    fdireccion[index]:=direccion;
    fTipoAnimacion[index]:=tipoAnimacion;
    fFrame[index]:=frame;
    inc(index);
  end;
begin
  if TipoAnimacion = fxSangre then
  begin
    setData(posLibre);
  ...
  begin
    setData(posLibre2);
    if posLibre2>MAX_SPRITES_FX2 then posLibre2:=BASE_SPRITES_FX2;
  end;
end;
...
begin
  for i:=0 to MAX_SPRITES_FX2 do
  begin
    if (fTipoAnimacion[i]=FX_Desactivado) or (fframe[i]<0) then continue;

    monstruo:=fmonstruo[i];
    if monstruo=nil then posicion_fx_Y:=fcoord_fx_y[i] else posicion_fx_Y:=monstruo.coordy;
    if (posicion_fx_Y<>fx_y) then continue;

    if monstruo=nil then posicion_fx_X:=fcoord_fx_x[i] else posicion_fx_X:=monstruo.coordx;
    if (abs(posicion_fx_X-fx_x)>=MaxVisionX) then continue;

    if not (animas.animacion[fTipoAnimacion[i]] is TAnimacionEfecto) then continue;
    animacion:=TAnimacionEfecto(animas.animacion[fTipoAnimacion[i]]);

    x:=centro_fx_x;
    y:=centro_fx_y;
    if monstruo=nil then
    begin
      inc(x,posicion_fx_X*ancho_tile);
      inc(y,posicion_fx_Y*alto_tile);
    end
    else with monstruo do
    begin
      inc(x,coordx*ancho_tile+(coordx_ant-coordx)*Paso_InterpoladoX*control_movimiento);
      inc(y,coordy*alto_tile+(coordy_ant-coordy)*Paso_InterpoladoY*control_movimiento);
    end;
    ...
    case fTipoAnimacion[i] of
      ...
      fxSangre:if monstruo<>nil then
        case InfMon[monstruo.tipoMonstruo].estiloMuerte of
          emSangreNegra:
...
function TJugador.ListarPericias:Tcadena127;
var i:integer;
begin
  for i:=0 to 15 do
...
function Tjugador.NombreCategoria:string;
begin
