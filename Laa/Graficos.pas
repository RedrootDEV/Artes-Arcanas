...
var
  ...
  PosicionRaton_SX,PosicionRaton_SY:smallint;
...
begin
  case akmonstruo.accion of
    ...
    aaAtacando1..aaAtacando5:begin
      result:=(akMonstruo.ritmoDeVida shr (Desplazador_AniSincro-1)) and $3;
    ...
    if bytebool(flagsPorSprite and fgfx_Transparencia) then
      if (flagsPorSprite and fgfx_Transparencia)=fgfx_Transparencia then
        ...
        if bytebool(FlagsPorGrafico and fgfx_Antialisado) then
          if bytebool(FlagsPorGrafico and dg_PermiteAutoTransparencia) and PuntoDentroRect(PosicionRaton_SX,PosicionRaton_SY,rdestino) then
            BltTrans(rdestino,Superficie,rorigen,espejo)
          else
