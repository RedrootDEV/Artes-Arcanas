...
const
  versionLA:byte=5;
  MAX_ARTEFACTOS=29;//En inventario
  ...
  i_ObjetoReparableVaEnManoDer=15;
  i_ObjetoNoReparable=16;
  ...
  i_NecesitasMasPlata=28;
  i_NecesitasMasOro=29;
  ...
  i_NoPuedesTeletransportarte=136;
  ...
  i_ComandoAdministrativoEjecutado=199;  
...
type
  ...
  TEstadoUsuario=(euNoConectado,euNoAutentificado,euAutentificado,euBaneado,euNormal,euModerador,euAdmin,euGameMaster);
  TDatosUsuario=record
    version:byte;
    EstadoUsuario:TEstadoUsuario;//1 byte
    DiaDeCreacion:word;//2 bytes
    ...
    Password:TSha256Hash;//32 bytes

    // TODO: Mejorar esto:
    IdentificadorDeClan:integer;//hace mas dificil explotar la colision de codigo de clan
    IdentificadorDeServidor:Integer;//Inicializado al azar, para detectar cambio de servidor
    // No es necesario guardar en archivo:
    ProcesarBufferRecepcion:bytebool;
...
// Daño: Determina si el modificador es de daño/armadura si es 1 (+$10), si es 0 es de ataque/evasión.
...
// Tipo: Determina el tipo de efecto mágico, va de 0 a 31 (+$1F)
  ...
  orUmbo=80;
  //Objetos
  ObNulo:TArtefacto=(id:0;modificador:0);
  ObManoDerVacia:TArtefacto=(id:0;modificador:1);//y mano derecha vacia
  ObManoIzqVacia:TArtefacto=(id:1;modificador:1);
  ObManoIzqOcupada:TArtefacto=(id:3;modificador:1);
  obCalderoMagico:TArtefacto=(id:ihCalderoMagico;modificador:0);
  obVaritaVacia:TArtefacto=(id:ihVaritaVacia;modificador:0);

// IDENTIFICADORES DE RECURSOS:
  SIN_RECURSOS=0;// $ff tambien significa sin recursos
  irLenna=1;
  irMadera=2;
  irMaderaMagica=3;
  irHierro=4;
...
function doubleaStr(valor:double):string;
function GetVersionCliente():string; overload;
function GetVersionCliente(version:byte):string; overload;
...
function BendecirObjeto(var Objeto,objEnManoDer:Tartefacto):byte;
...
function EsIdArmaduraMetalica(objeto:Tartefacto):bytebool;
...
//function caosHash(x, y: cardinal):cardinal; register;
function jenkinsHash(x, y: cardinal):cardinal; register;
procedure SetFastRandomSeed(seed:cardinal); register;
function XRandom():cardinal; overload; register;
function XRandom(max:integer):integer; overload; register;
function XRandom(max:integer; var seed:integer):integer; overload; register;
...
function caosHash(x, y: cardinal):cardinal; register;
begin
  result:= x*374761393 + y*668265263;
  result:= (result xor (result shr 13))*1274126177;
...
function XRandom(max:integer):integer; overload; register;
// max en EAX, result en EAX
asm
  mov edx, g_fast_random_seed
...
function XRandom(max:integer; var seed:integer):integer; overload; register;
// max en EAX, result en EAX, @seed en EDX
asm
  push ebx
  mov ecx, [edx]
  mov ebx, edx
...
function GetVersionCliente():string;
begin
...
begin
  ...
  begin
    ...
    result:=((tipo_municion>=5)and(tipo_arco>=5)) or (tipo_municion=tipo_arco);
  end
  else
    result:=false;
end;
...
function CalcularModificadorAtaDef(const objeto:TArtefacto):integer;
//Calcula modificador de ataque/evasión
//No controla el tipo de objeto
var tipoMagia:TTipoMagiaArtefacto;
begin
  if(objeto.modificador=0) then
  begin
    result:=0;
    exit;
  end;
...
begin
  result:=boNinguno;
  case objeto.id of
    16..47,56..103,248..253://Artefactos benditos/malditos por bono en daño,ataque,evasión.
    begin
      EstadoObjeto(objeto.modificador,tipoMagia,modificaDanno,esMalvado,estaIdentificado);
      if (tipoMagia=maNinguna) then
      begin
        if objeto.modificador=0 then result:=boMalvado;
      end
      else
...
begin
  case objeto.id of
    ...
    4..15,ihPlumaMagica,144..175,192..231://Objetos con cantidad 1byte;
...
begin
  ...
  case MaximaCantidadDeEsteObjeto(objeto) of
    ...
    begin
      ...
      estado:=2;
      if bytebool(objeto.modificador and MskEnvenenado) then inc(estado);
      if bytebool(objeto.modificador and MskParalizante) then inc(estado,2);
      result:=(result*estado) shr 1;
    end;
    toCantidad_250:
    begin
      ...
      case objeto.id of
        ...
        //Consumibles
        136..143:begin
          estado:=InfObj[objeto.id].modificadorADC;
          if (objeto.modificador>estado) then estado:=objeto.modificador;
        ...
        begin
          ...
          case tipoMagia of
            ...
            maModificador, maElemento:
              if estaIdentificado then
                if esMaldito then
                begin
                  estado:=8-(objeto.modificador and MskEstadoMagico);
                  result:=(result*estado) shr 4;
                end
                else
                begin
                  estado:=2+(objeto.modificador and MskEstadoMagico);
                  result:=(result*estado)+InfObj[orOricalco].costo;
                end;
            maHechizo:
              if estaIdentificado then result:=(result shl 2);
          end;
        end;
      end;
  end;
...
//SoloObjetosEnBuenEstado=true => De los hechizables sólo los modificadores de 25 a 63, de las gemas los modificadores de 25 a 100
...
begin
  ...
  begin
    ...
    if (objEnManoDer.id>=orGemaHielo) and (objEnManoDer.id<=orDiamante) then
    begin
      if (objEnManoDer.id=orDiamante) and EsIdDeArmadura(id) then
      begin
        result:=i_ElBrillanteNoAfectaArmaduras;
        exit;
      end;
      if (objEnManoDer.modificador>=50) and
        (objEnManoDer.modificador<=100) then
      begin
        case objEnManoDer.modificador of
          80..90:nivelBendicion:=1;
          91..94:nivelBendicion:=2;
          95..96:nivelBendicion:=3;
          97:nivelBendicion:=4;
          98:nivelBendicion:=5;
          99:nivelBendicion:=6;
          100:nivelBendicion:=7;
        end;
        case objEnManoDer.id of
        ...
        objEnManoDer:=ObManoDerVacia;
        result:=i_ok;
      end
      else
      begin
        if (objEnManoDer.modificador>100) then
          result:=i_EstaGemaNoEsAdecuada
        else
          result:=i_NecesitasGema50a100;
        exit;
      end;
    end;
    if (objEnManoDer.id=orOricalco) and (objEnManoDer.modificador>0) then
    begin
      case objEnManoDer.modificador of
      //mucho ojo con esto!!!
        1..3:begin
      ...
      dec(objEnManoDer.modificador,nivelDeConsumo);
      if objEnManoDer.modificador=0 then
        objEnManoDer:=ObManoDerVacia;
...
begin
  result:=(((armadura.id>=56) and (armadura.id<=97)) or ((armadura.id>=248) and (armadura.id<=253)))
    and (armadura.modificador>0) and (armadura.modificador<=MskEstadoObjetoNormal);
  if result then
  begin
    if (armadura.modificador>danno) then dec(armadura.modificador,danno) else armadura.modificador:=0;
  end;
end;
...
begin
  ...
  if result then
    if (arma.modificador>0) then
    begin
      dec(arma.modificador);
      if esArmaConjurada and (arma.modificador=0) then arma.id:=0;
    end;
end;
...
function EsIdArmaduraMetalica(objeto:Tartefacto):bytebool;
begin
  result:=(objeto.id>=62) and (objeto.id<=71) and (objeto.modificador>0);
end;
