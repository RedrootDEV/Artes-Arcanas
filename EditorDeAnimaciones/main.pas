...
type
  ...
  TForm1 = class(TForm)
    ...
    Crearanimacionbandera: TMenuItem;
    ...
    E_alinlinx: TEdit;
    E_alinliny: TEdit;
    Alinearanimacinconunasoladireccin1: TMenuItem;
    ...
    procedure E_alinlinxChange(Sender: TObject);
    procedure E_alinlinyChange(Sender: TObject);
    procedure Alinearanimacinconunasoladireccin1Click(Sender: TObject);
    procedure CrearanimacionbanderaClick(Sender: TObject);
  private
...
begin
  ...
  ResultadoIndexed:=Tbitmap.create;
  ColTrans:=C_tran;
  CB_dir.ItemIndex:=0;
...
begin
  ...
  begin
    ...
    E_alinlinx.text:=intastr(modix);
    E_alinliny.text:=intastr(modiy);
    DibujarPantalla;
    Pantalla.update;
  end;
end;
...
procedure TForm1.Alinearanimacinconunasoladireccin1Click(Sender: TObject);
...
procedure TForm1.E_alinlinxChange(Sender: TObject);
begin
  with D[DirAlin].alinlin do
  begin
    modix:=strtoint(E_alinlinx.text);
    E_alinlinx.text:=intastr(modix);
  end;
  DibujarPantalla;
  Pantalla.update;
...
procedure TForm1.E_alinlinyChange(Sender: TObject);
begin
  with D[DirAlin].alinlin do
  begin
    modiy:=strtoint(E_alinliny.text);
    E_alinliny.text:=intastr(modiy);
  end;
  DibujarPantalla;
  Pantalla.update;
...
procedure TForm1.CrearanimacionbanderaClick(Sender: TObject);
var j,M_frames,minVal,maxVal:integer;
    alin2:TAlinLin2;
    f1:file;

    procedure CreateIndexed();
    type TWinPalette=packed record
           palVersion: Word;
           palNumEntries: Word;
           data:array[0..255] of TPaletteEntry;
         end;
    var pal:TWinPalette;
        i:integer;
    begin
      with ResultadoIndexed do
      begin
        pal.palversion := $300;
        ...
        for i:=0 to 255 do
          ...
          begin
            peRed:=(i shr 2) and 3;
            peGreen:=i and 3;
            peBlue:=(i shr 4) and $f;
            peFlags:=0;
          end;
        PixelFormat:=pf8bit;
        if (Palette<>0) then
          SetPaletteEntries(Palette,0,256,pal.data)
        else
    ...
    procedure getRange(var min,max:integer);
    var i,j:integer;
        linea:Plinea;
        v:byte;
    begin
        ...
        if not Empty then
          ...
          begin
            ...
            begin
              ...
              begin
                v:=linea[i].r;
                if (v<linea[i].g) then v:=linea[i].g;
                if (v<linea[i].b) then v:=linea[i].b;
                if (v > 0) then
                begin
                  if (v < min) then min:=v;
                  if (v > max) then max:=v;
    ...
    procedure copyToIndexed(min,max:integer);
    var i,j:integer;
        linea:Plinea;
        linea8:Plinea8;
        c,v:byte;
        function GetCategory(pixel:T3RGB):integer;
        begin
          {
          0:white
          1:red
          ...
          3:cyan: blue green
          4:green
          5:orange: red 2 green 1
          6:magenta: blue red
          7:yellow
          8:sky: blue 2 green 1
          9:violet: blue 2 red 1
          10:black
          }
          if (pixel.r>0) then
          begin
          ...
          begin
            if (pixel.g>0) then
            begin
              if (pixel.b>pixel.g) then
              begin
                result:=8;
                exit;
              end
              else if (pixel.b>0) then
              begin
                result:=3;
                exit;
              end
    ...
    begin
      with ResultadoFinal do
        if not Empty then
          if (Width=ResultadoIndexed.Width) and (Height=ResultadoIndexed.Height) then
          begin
            for j:=0 to Height-1 do
            begin
              linea:=ScanLine[j];
              linea8:=ResultadoIndexed.ScanLine[j];
              for i:=0 to Width-1 do
              begin
                c:=GetCategory(linea[i]);
                if (c=10) then
                begin
                   linea8[i]:=0;
                end
                ...
                begin
                  ...
                  if (v<linea[i].b) then v:=linea[i].b;
                  // min..max to 1..15
                  linea8[i]:=c*16 + (((v-min)*14) div (max-min)) + 1;
                end;
              end;
            end;
          end;
...
begin
  M_frames:=d[DirAlin].MaxImagenesListas;
  if d[DirAlin].ImagenesListas and (M_frames=MaxCuadrosBandera) then
  begin
  // Componer el resultado final.
  //*************************************************
    if GuardararchivoBMP1.checked then
    begin
      ...
      begin
        createIndexed();
        copyToIndexed(minVal,maxVal);
        ResultadoIndexed.SaveToFile(nombreAR+'bmp');
      end
      else
      begin
        showmessage('Imagen incorrecta.');
      end;
    end;
    ...
    showmessage('Las banderas deben tener 8 cuadros de animacion.');
end;
