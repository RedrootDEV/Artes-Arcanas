...
const
  //Archivo de log.
  NOMBRE_ARCHIVO_LOG='server.log';
  MAX_TAMANNO=30000;//256000;
  CACHE_MENSAJES=256;//128;
  CACHE_MENSAJES2=CACHE_MENSAJES shr 1;
  MAX_TAM_MENSAJE_SERVIDOR=255;

  //No modificar, tiene que ser 255
  MAX_TIEMPO_OCIO=255;
  TIEMPO_ANTES_DE_DESCONECTAR=MAX_TIEMPO_OCIO shr 2;
  //Modificables
  MENSAJE_TURNO_EN_PROGRESO='Intente nuevamente, el servidor está ocupado.';
...
//En todo el mundo:
  ...
  //--------------------------------------------------
  procedure ActivarBerserker(Jug:TjugadorS);
...
//  procedure RealizarResurreccionYNotificar();
  ...
  procedure IntercambiarObjetoEnBaul(Jug:TjugadorS;indice1,indice2:byte);
  ...
  procedure EliminarCadaveres(jug:TjugadorS);
  procedure EliminarBolsas(jug:TjugadorS);
  procedure RealizarYNotificarResureccionAvatar(jug:TjugadorS);
  ...
  procedure DetenerMovimiento(Jug:TjugadorS);
...
var
  g_inicializado,g_realizando_turno:boolean;
...
//Inicialización del servidor
  mensaje('Bienvenido al mundo de Artes Arcanas');
...
begin
  ...
  if TamArchivo<0 then//el archivo no existe
    ...
    begin
      {$I-}
      AssignFile(f,DirArchivoLog+NOMBRE_ARCHIVO_LOG);
      Rename(f,DirArchivoLog+'server.'+IntToHex(trunc(date)-38154,5)+'.log');
      {$I+}
...
//REGENERACION DE HP Y MANA.
    ...
    var hpTempo,bono:integer;
    begin
      result:=false;
      if JugCon.hp=0 then exit;
      ...
      with JugCon do
      begin
        if (hp=maxhp) then exit;

        hpTempo:=0;
        if (accion=aaDescansando) then
        begin
          bono:=1;
          if longbool(banderas and BnVendado) then inc(bono);
          if (aurasExternas and flAuraExtFogata)<>0 then inc(bono);
          if (hp>=(maxhp shr 2)) then inc(bono);
          inc(hpTempo,bono);
          inc(hpTempo,(maxhp * bono) shr 6);
        end;

        if (Usando[uAmuleto].id=ihAmuletoDeRegeneracion) then
        begin
          inc(hpTempo);
          if (CON=20) then inc(hpTempo);
        end;
        if ((Pericias and hbRegenerar)<>0) then
        begin
          inc(hpTempo);
          if (CON>=16) then inc(hpTempo);
        end;
    ...
    begin
      result:=false;
      with JugCon do
      if (maxMana>0) and (comida>0) and (mana<maxmana) then
      begin
        if (accion=aaMeditando) and (Mapa[codMapa].SePuedeMeditarDescansar(coordx,coordy)) then
        begin
          ...
          if codCategoria=ctMago then inc(aumentado);
          //Si esta cerca de fogata y meditando aumentar en 50%
          if ((aurasExternas and flAuraExtFogata)<>0) then
          begin
            inc(aumentado,(aumentado+1) shr 1);
            inc(aumentado,Meditacion255);
          end;
          inc(aumentado,mana);
          if aumentado>maxmana then aumentado:=maxmana;
          mana:=aumentado;
          result:=true;
        end;
        if (Usando[uAmuleto].id=ihAmuletoDeMago) then
        begin
          aumentado:=mana+1;
          if (INT=20) then inc(aumentado);
          if (SAB=20) then inc(aumentado);
          if aumentado>maxmana then aumentado:=maxmana;
          mana:=aumentado;
          result:=true;
        end;
    ...
    function consumirComida:boolean;
    var puntosConsumidos:integer;
    begin
      result:=JugCon.comida>0;
      ...
      begin
        ...
        if (CON>=10) then
        begin
          inc(puntosConsumidos);
          if (CON=20) then inc(puntosConsumidos);
        end;
        if comida>puntosConsumidos then
          dec(comida,puntosConsumidos)
        else
          comida:=0;
      end;
    end;
  ...
  for i:=0 to maxJugadores do
    with Jugador[i] do
      ...
      begin
        ...
        begin
          if longbool(banderas and bnApresurar) or ((banderas and bnCongelado)=0) or ((conta_i and $1)=0) then
          begin
            MoverAutomaticamente(JugCon);
            RealizarControlSensores(JugCon);
          end;
          if ((coordX<>fdestinoX) or (coordY<>fdestinoY)) and longbool(banderas and bnApresurar) and ((banderas and bnCongelado) = 0) then
          begin
            MoverAutomaticamente(JugCon);
            RealizarControlSensores(JugCon);
          end;
        ...
        begin
          ...
          begin
            ...
            if conta_i and $7F=0 then
              if consumirComida() then
...
begin
  ...
  pendienteClimaGeneral:=PendienteDeClima(Tipo);
  for i:=0 to maxMapas do
    with Mapa[i] do
    begin
...
var temp,nuevaIntensidad:integer;
begin//iniciador de eventos del clima
      ...
      begin//Efectos varios
        ...
        begin
          ...
          begin
            ...
            if (IntensidadClimaGeneral=255) and (random(4)=0) then
              finalizarClima
            else
              if (TipoClimaGeneral=CL_LLUVIOSO) and (random(8)=0) then
                EnviarATodos('XR');
        end;
      end
      else
  ...
  begin
    nuevaIntensidad:=IntensidadClimaGeneral+pendienteClimaGeneral;
    if nuevaIntensidad<=0 then
    begin
      pendienteClimaGeneral:=0;
    ...
    else if nuevaIntensidad>=255 then
    begin
      IntensidadClimaGeneral:=255;
      pendienteClimaGeneral:=0;
    end
    else
      IntensidadClimaGeneral:=nuevaIntensidad;
  end;
  //Control de timer
...
begin
  ...
  case Conta_Universal and $7 of
    0:begin//Monstruos definidos en el mapa en nidos.
      for i:=0 to conta_Monstruos_Definidos-1 do
        with monstruo[i] do
          if not activo then
            Mapa[codMapa].EngendrarMonstruo(monstruo[i])//Intentar crear un nuevo monstruo.
    end;
    2:begin//Monstruos conjurados por magos/clérigos
      ...
      with monstruo[i] do
        if activo then
          if (hp>0) then
            ...
            begin//regeneracion:
              if ((banderas and bnEnvenenado)=0) then
              begin
                if (hp<HPPromedio) then
                begin
                  NivelTemporal:=hp;
              ...
              begin
                if (Regeneracion>random(30)) then
                begin
              ...
              //mana para monstruos:
              NivelTemporal:= nivelMonstruo shl 1;
              if (mana<NivelTemporal) and (mana<250) then inc(mana);
            end;
    end;
  end;//case
end;
  ...
  for i:=0 to maxJugadores do
    if SocketDelJugador[i]<>nil then
      if Jugador[i].activo then
        ...
        begin
          if DatosUsuario[i].TimerDesconeccionPorOcio=(TIEMPO_ANTES_DE_DESCONECTAR+1) then
            SendText('I'+#0);
          SendBufferedTextNow;
        end;
...
function Mundopreparado:boolean;
begin result:=g_inicializado end;
...
begin
  ...
  begin
    ...
    if SSocket.Active then
      result:='Nombre del servidor: '+SSocket.Socket.LocalHost+' IP: '+SSocket.Address+':'+intastr(SSocket.port);
...
begin
  ...
  Timer.enabled:=false;
  if not g_realizando_turno then
  begin//Es seguro realizar el almacenamiento
    ...
    mensaje(MENSAJE_TURNO_EN_PROGRESO);
...
var cantidadDisponible:integer;
  ...
  begin
    if consumidor<maximoConsumidor then
      ...
      begin
        dec(consumido);
        NuevoNivel:=consumidor+PUNTOS_HP_POCION;
  ...
  begin
    if consumidor<maximoConsumidor then
      ...
      begin
        dec(consumido);
        NuevoNivel:=consumidor+PUNTOS_MANA_POCION+bonoConsumo;
        if NuevoNivel>maximoConsumidor then
          NuevoNivel:=maximoConsumidor;
        consumidor:=NuevoNivel;
        result:=i_OK;
...
begin
  ...
  begin
    ...
    Case idObj of//clase de objeto
      ...
      begin
        if(infobj[idObj].modificadorADC = (infobj[idObj].costo shl 2)) then
        begin
          if Mapa[codmapa].ObtenerRecursoAlFrente(Jug)=irFogata then
          begin
            Artefacto[IndArt].id:=144;
            //Sonido de cocinar
            EnviarAlAreaMonstruo(jug,'S'+char(jug.coordx)+char(jug.coordy)+#7);
            exit;
          end;
          if (comida>25) then exit;
        end;
        ...
        case idObj of
          ...
          orBebidaMasMANA:
          begin
            IncrementarConLimite(mana,maxMana,4);
            SendText(codigo,#254+char(mana));
          end;
          orBebidaMasHP:
          begin
            IncrementarWordConLimite(hp,maxHp,6);
            SendText(codigo,#255+B2aStr(hp));
          end;
          orBebidaMasManaHp:
          begin
            IncrementarWordConLimite(hp,maxHp,10);
            IncrementarConLimite(mana,maxMana,5);
            SendText(codigo,#255+B2aStr(hp));
            SendText(codigo,#254+char(mana));
          end;
        end;
...
var i:integer;
  ...
  begin
    with Jug.Baul[i] do//Notificar al cliente del objeto del baul de castillo
      SendText(Jug.codigo,'b'+char(i)+char(id)+char(modificador));
  end;
begin
  result:=true;
        ...
        for j:=0 to 2 do//Verificar con cada recurso necesario
          if Contadores[j]>0 then
            ...
            begin
              ...
              begin
                cadena:=cadena+char(i+8{Es del inventario}+208{Refrescar Objetos})+
...
begin
  with Jug do
  if longbool(pericias and hbBerserker) and (hp>PENA_HP_BERSERKER) then
  begin
    dec(hp,PENA_HP_BERSERKER);
    ...
    if nivel>MAX_NIVEL_NEWBIE then
      ...
      tiempo:=3;
    inicializarTimer(tdBerserker,tiempo);
    EnviarAlMapa_J(Jug,'A'+b2aStr(codigo)+char(banderas));
  ...
  begin
    ...
    begin
      ...
      begin
        danno:=danno shr 1;
        with jug do
          if Usando[elementoDannado].modificador=0 then
          begin
            CalcularModDefensa;
            determinarAnimacion;
            EnviarAlMapa(codMapa,'F'+B2aStr(codigo)+char(codAnime));
          end;
        SendText(jug.codigo,char(ord(#0)+elementoDannado)+char(jug.Usando[elementoDannado].modificador));
      end
    end;
  end;
...
procedure IntercambiarObjetoEnBaul(Jug:TjugadorS;indice1,indice2:byte);
var ObjetoTemporal:TArtefacto;
begin
  if (indice1>MAX_ARTEFACTOS) or (indice2>MAX_ARTEFACTOS) or (indice1=indice2) then exit;

  with Jug do
  begin
    ObjetoTemporal:=Baul[indice1];
    Baul[indice1]:=Baul[indice2];
    Baul[indice2]:=ObjetoTemporal;
    SendText(Jug.codigo,'b'+char(indice1)+char(Baul[indice1].id)+char(Baul[indice1].modificador)+
      'b'+char(indice2)+char(Baul[indice2].id)+char(Baul[indice2].modificador));
  end;
end;
...
begin
  with Jug do
  if ((banderas and bnParalisis)=0) and (indArtefacto<=MAX_ARTEFACTOS) then
    with Mapa[codmapa] do
  ...
  if ((banderas and bnParalisis)=0) and (indArtefacto<=MAX_ARTEFACTOS) then
    ...
    begin
      ...
      with Baul[indArtefacto] do
        if (id<>EstdAntrr.id) or (modificador<>EstdAntrr.modificador) then
          sendText(codigo,'b'+char(indArtefacto)+char(id)+char(modificador));
    end;
end;
...
var controlParty:word;
  ...
  var i:integer;
      ...
      camarada:TjugadorS;
  begin
    if jug.nivel<=0 then exit;
    for i:=0 to MAX_INDICE_PARTY do
    begin
      codCasilla:=jug.CamaradasParty[i];
      if AsegurarCamaradaPartyValido(jug,codCasilla) then
      begin
        camarada:=Jugador[codCasilla];
        if (camarada.nivel>0) and (camarada.codMapa=jug.codMapa) and
          (abs(camarada.coordx - jug.coordx)<=MaxRangoParty) and
          (abs(camarada.coordy - jug.coordy)<=MaxRangoParty) then
        begin
          NotificarModificacionExperiencia(camarada,cantidadExp shr 1);
        end;
      end;
    end;
  ...
  for i:=conta_Monstruos_Definidos to Indice_Maximo_Monstruos do
    with Monstruo[i] do
      if duenno=AlineacionDelLider then
        if codMapa=codigoMapaDelLider then
          if activo then
            if (abs(coordx-coordxLider)<=MaxRangoParty) and (abs(coordy-coordyLider)<=MaxRangoParty) then
    ...
    begin
      with Mapa[codMapa] do
        ...
        begin
          ...
          TeletransPortarJugador(Jug,0,127,127);//Al infierno
        end
        else
          if ((Posx_Resucitar<>0) or (Posy_Resucitar<>0)) then
            if (banderas and bnControlado)=0 then
              TeletransPortarJugador(Jug,Mapa_Resucitar,Posx_Resucitar,Posy_Resucitar)
            else
            begin
      ...
      RealizarYNotificarResureccionAvatar(jug);
    end;
end;
...
begin
  ...
  SendText(Jug.codigo,'i'+char(i_ComandoAdministrativoEjecutado));
end;
...
begin
  ...
  begin
    Mapa[i].controlParaEliminarCadaveres:=MaxBolsas;
    Mapa[i].controlParaEliminarBolsas:=MaxBolsas;
  end;
  SendText(Jug.codigo,'i'+char(i_ComandoAdministrativoEjecutado));
end;
...
begin
  ...
  begin
    ...
    resucitar;
    determinarAnimacion;
    sendText(codigo,'srF'+B2aStr(codigo)+char(codAnime));
...
begin
  ...
  begin
    ...
    result:=(TiempoDeAtacar=0) or (((Conta_Universal + codigo) mod TiempoDeAtacar)=0);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      //Hechizos
      if ((monstruoAt.PericiasDinamicas and perMon__DisiparMagia)<>0) and
        ((Victima.banderas and MskBanderasPositivas)<>0) and (mana>=18) then
      begin
        dec(mana,18);
      ...
      if (victima is TJugadorS) and ((victima.banderas and bnAturdir)=0) and
        ((monstruoAt.PericiasDinamicas and permon__aturdir)<>0) and (mana>=30) and
        (victima.mana=0) then
      begin
        dec(mana,30);
      ...
      if ((Victima.banderas and bnParalisis)=0) and (mana>=25) and
        ((monstruoAt.PericiasDinamicas and permon__Paralizar)<>0) then
      begin
        dec(mana,25);
        ParalizarMonstruo(victima,3);
        exit;
      end;
    end;
  end;
