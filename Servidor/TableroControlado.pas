...
type
  ...
  public
    ...
    controlParaEliminarBolsas:integer;
    ...
    procedure PrenderFogata(x,y:byte;codigoJugador:word);
  end;
...
const
  DURABILIDAD_ARMAS_MELEE=5;
  DURABILIDAD_ARMAS_RANGO=8;
  DURABILIDAD_ARMAS_RANGO_EN_LLUVIA=4;
      ...
      begin
        ...
        if DuennoMonstruo=nil then
          ...
          begin
            ...
            //bono de vida
            inc(hp,TjugadorS(DuennoMonstruo).nivel shl 1);
  ...
  begin
    ...
    begin
      ...
      if TipoAlcance=aaRango then
        with Victima do
        begin
          distancia:=(sqr(coordx-MonstruoAt.coordx)+sqr(coordy-MonstruoAt.coordy)) shr 2;
    ...
    begin
      if AtaqueDeMonstruoConHechizos(monstruoAt,Victima) then
      begin
        result:=true;
        exit;
      end;
      if (MonstruoAt.mana=0) or (IdAtaqueElegido>MAX_TIPOS_ATAQUE_MONSTRUO) then
      begin
        result:=false;
        exit;
      end;
    ...
    if (lanzada>=NIVEL_ATAQUE_SIEMPRE_EXITOSO) or (
        (lanzada>=MINIMO_PORCENTAJE_DE_ATAQUE) and
        ((lanzada+InfMon[monstruoAt.TipoMonstruo].NivelAtaque)>=defensa_en_Combate)) then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if VictimaEsJugador then
            begin
              if (TjugadorS(Victima).PerderMeditarDescansar()) then
                EnviarAlMapa_J(TjugadorS(Victima),char((accion and mskAcciones)+160)+b2aStr(codigo));
              SendText(codigo,#252+B2aStr(hp)+char(cdNombre)+char(monstruoAt.tipoMonstruo));
            end;
          end;
        with Victima do
        begin
          if (Victima is TjugadorS) and EsIdArmaduraMetalica(TjugadorS(Victima).Usando[uArmadura]) then
...
var cadena:string;
    i,d,NuevoComportamiento:integer;
...
begin
  ...
  begin
    ...
    begin
      i:=(Victima.banderas and MskPoderMonstruo) shr DsPoderMonstruo;
      i:=((2+i)*Pexperiencia) shr 1;
      if JugadorAt.nivel>nivelMonstruo then
      begin
        d:=16-(JugadorAt.nivel-nivelMonstruo);
        if (d < 4) then d:= 4;
        i:= (i * d) shr 4;
      end;
      if i<2 then i:=2;
  ...
  with JugadorAt do
  begin
    AnteriorEspecialidadArma:=EspecialidadArma and mskEspecialidadObjeto;
    AnteriorNivelEspecialidad:=NivelEspecializacion and $FC;
    //Ojo que no debe existir otra función que incremente/decremente "Nivel Especialización"!!!
    if AnteriorEspecialidadArma=idArma then
    begin
      if NivelEspecializacion<=251 then
        if nivel>=MIN_NIVEL_CATEGORIA then
          inc(NivelEspecializacion,4)
        else
          inc(NivelEspecializacion,2)
      else
        NivelEspecializacion:=255;
      if (AnteriorNivelEspecialidad<>(NivelEspecializacion and $FC)) then
        cadena:=cadena+'Ie'+char(NivelEspecializacion);
    end
    else if (EspecialidadArma and mskEspecialidadFija)=0 then
    begin
      if NivelEspecializacion>16 then
      begin
        dec(NivelEspecializacion,16);
        if (AnteriorNivelEspecialidad<>(NivelEspecializacion and $FC)) then
          cadena:=cadena+'Ie'+char(NivelEspecializacion);
      end
      else if (AnteriorEspecialidadArma>=16) and (idArma>=16) and (idArma<48)  then
      begin
        EspecialidadArma:=idArma or mskEspecialidadFija;
...
begin
  ...
  if MonstruoAt is TJugadorS then
    if TjugadorS(MonstruoAt).ConjuroElegido<3 then
      PrenderFogata(posConjuro_x,posConjuro_y,ccVac);
...
begin
  ...
  if MonstruoAt=MonstruoVictima then exit;
  ...
  begin
    ...
    begin
      ...
      if VictimaEsJugador then
        ...
        begin
          if (TjugadorS(MonstruoVictima).PerderMeditarDescansar()) then
            EnviarAlMapa_J(TjugadorS(MonstruoVictima),char((accion and mskAcciones)+160)+b2aStr(codigo));
  ...
  var nuevaDireccion:TDireccionMonstruo;
    ...
    var ExperienciaGanada:integer;
      ...
      begin
        ...
        //Los clerigos curan hasta 4hp por punto de mana
        ...
        begin
          ...
          begin//optimizado con desplazadores, sin riesgo de op. en negativos!
            ...
            // -17 para sacar por lo menos un punto de mana
          ...
          begin//optimizado con desplazadores, sin riesgo de op. en negativos!
            ...
            if MaxHPCurablePorMana>20 then MaxHPCurablePorMana:=20;
            if hpACurar>MaxHPCurablePorMana then hpACurar:=MaxHPCurablePorMana;
            // -5 para sacar por lo menos un punto de mana
      ...
      begin
        ...
        with TjugadorS(M_Objetivo) do
          if (hp=0) then
          begin
            RealizarYNotificarResureccionAvatar(TjugadorS(M_Objetivo));
            if (JugadorAt.CodCategoria<>ctClerigo) then
            begin
              JugadorAt.hp:=1;
    ...
    begin
      ...
      begin
        ...
        begin
          ExperienciaGanada:= 16 - (nivel-nivelJugador);
          if ExperienciaGanada<4 then ExperienciaGanada := 4;
          ExperienciaGanada:=((nivelINT+nivelSAB+nivelMana+nivel)*ExperienciaGanada) shr 4;
        end;
  ...
  begin
    ...
    begin
      ...
      begin//conjuro lanzado a un objetivo.
        ...
        nuevaDireccion:=calcularDireccion(apuntado.coordx-coordx,apuntado.coordy-coordy);
...
var Ataque_Combate:integer;
  ...
  begin
    ...
    begin
      ...
      nuevaDireccion:=calcularDireccion(deltaX,deltaY);
      if dir<>nuevaDireccion then
      begin
    ...
    //Salir si el camino al monstruo está bloqueado.
      ...
      SendText(codigo,char(ord(#0)+uMunicion)+char(municion.modificador));
      if ObjetoArma.id=idArcabuz then SonidoArma:='A' else
        if ObjetoArma.id=idHonda then SonidoArma:='h' else SonidoArma:='R';
    ...
    begin
      ...
      distancia:=(deltax*deltax+deltay*deltay) shr 2;
      inc(defensa_en_Combate,distancia+modificadorClima(jugadorAt.tieneInfravision())-tamanno*5);
      if (jugadorAt.EspecialidadArma and mskEspecialidadObjeto)=ObjetoArma.id then
        dec(defensa_en_Combate,(JugadorAt.NivelEspecializacion shr 2)+1);
      ...
      begin
        ...
        if (jugadorAt.EspecialidadArma and mskEspecialidadObjeto)=ObjetoArma.id then//Bono especialidad
          inc(danno,(JugadorAt.NivelEspecializacion+4) shr 3);
        ...
        if bytebool(municion.modificador and mskParalizante) then
          if ((municion.id=ihDardoParalisis) and (random(2)=0)) or (danno>=10)  then
        ...
        if controlArmaDannada(ObjetoArma,DurabilidadArma) then
          SendText(JugadorAt.codigo,char(ord(#0)+idPosArma)+char(ObjetoArma.modificador));


        if (ObjetoArma.id=idArcabuz) or (ObjetoArma.id=idHonda) then SonidoArma:='C' else SonidoArma:='F';
        if (Victima is TjugadorS) and EsIdArmaduraMetalica(TjugadorS(Victima).Usando[uArmadura]) then
      ...
      begin
        with jugadorAt.usando[uAmuleto] do
          if id=orAmuletoGuerrero then //amuleto guerrero
            dec(defensa_en_Combate,(modificador and MskEstadoObjetoNormal) shr 2);
        if (jugadorAt.EspecialidadArma and mskEspecialidadObjeto)=ObjetoArma.id then//Especialidad
          dec(defensa_en_Combate,(JugadorAt.NivelEspecializacion shr 2)+1);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (EspecialidadArma and mskEspecialidadObjeto)=ObjetoArma.id then
                inc(danno,(NivelEspecializacion+4) shr 3);
            ...
            begin
              inc(danno, DES);
              // Para bribones doble bono
              if (CodCategoria=ctBribon) then inc(danno, nivel shr 2);
            end;
          end;
          //Modificadores de daño:
          ...
          begin
            ...
            if (ObjetoArma.id=0) then
              if idPosArma=uArmaDer then
                ObjetoArma:=ObManoDerVacia
              else
                ObjetoArma:=ObManoIzqVacia;
          end;
    ...
    for idPosArma:=uArmaIzq downto uArmaDer do
      ...
      begin
        ...
        if cdErrorAlApuntar<>i_Ok then
          ...
          if MunicionCorrecta(Usando[idPosArma],Usando[uMunicion]) then
            if Usando[idPosArma].modificador > 0 then
              cdError:=realizarAtaqueRango(Usando[idPosArma],Usando[uMunicion])
            else
              cdError:=i_ArmaDestrozada
          else
      ...
      begin
        ...
        if InfObj[Usando[idPosArma].id].AlcanceArma=aaMelee then
          ...
          begin
            ArmaRota:=ObManoDerVacia;
            cdError:=realizarAtaqueMelee(ArmaRota);
          end;
      end;
...
begin
  ...
  begin
    ...
    tempx:=coordX;
    tempy:=coordY;
    dirDestino:=calcularDireccion(fdestinox-coordx,fdestinoy-coordy);
...
begin
  ...
  begin
    ...
    begin
      with Bolsa[controlParaEliminarCadaveres] do
        if (tipo>=tbCadaver) and (tipo<=tbCenizas) and (tipo<>tbCadaverAvatar) then
        begin
          tipo:=tbNinguna;
          mapaPos[posx,posy].terBol:=mapaPos[posx,posy].terBol or mskBolsa;
  ...
  if (controlParaEliminarBolsas>=0) then//eliminar bolsas
  begin
    i:=0;
    while i<16 do //eliminar 16 bolsas en cada turno
    begin
      with Bolsa[controlParaEliminarBolsas] do
        if BolsaSinContenidoImportante(controlParaEliminarBolsas) then
        begin
          tipo:=tbNinguna;
          mapaPos[posx,posy].terBol:=mapaPos[posx,posy].terBol or mskBolsa;
          EnviarAlMapa(fcodMapa,#192{Eliminar bolso}+char(posx)+char(posy));
      ...
      dec(controlParaEliminarBolsas);
      if controlParaEliminarBolsas<0 then break;//salir del ciclo
    end;
  end;
end;
...
var posAFM_x,posAFM_y:smallint;//Posición al frente del monstruo
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          dir:=calcularDireccion(MonstruoObjetivo.coordx-coordx,MonstruoObjetivo.coordy-coordy);
          exit;
        end
      end;
      while (conta<=3) do
  ...
  function viaLibreParaAtaque(monstr:TmonstruoS;xd,yd:byte):boolean;
  var tempDir:TDireccionMonstruo;
  begin
    result:= false;
    tempDir:=calcularDireccion(xd-monstr.coordx,yd-monstr.coordy);
    if (monstr.dir<>tempDir) then
    begin
      monstr.dir:= tempDir;
      exit;
    end;
    if ExisteObstaculo(monstr.coordx,monstr.coordy,xd,yd) then exit;
    result:= true;
  end;
    ...
    if victima<>nil then
      with victima do
        ...
        begin//vivo y visible
          ...
          begin
            if (distancia>RangoArqueroEnNormaCuadrado) and (monstr.hp>(InfMon[monstr.TipoMonstruo].HPPromedio shr 1)) then
              BuscarCaminoParaAvanzar(false,true)
            else
            begin
              if viaLibreParaAtaque(monstr,coordx,coordy) then
                if ataqueMonstruo(monstr,victima,aaRango,SeleccionarAtaqueMonstruo) then exit;
              if (distancia<2) then
              begin
                P_DarVueltaDireccion(monstr.dir);//escapar
                BuscarCaminoParaAvanzar(true,true);
              end
              else if (distancia>3) then
                BuscarCaminoParaAvanzar(false,true);
            end;
          end;
      ...
      with victima do
        ...
        begin//vivo y visible
          ...
          begin
            ...
            begin
              if viaLibreParaAtaque(monstr,coordx,coordy) then
                if ataqueMonstruo(monstr,victima,aaMagica,AtaqueElegido) then
                begin
                  result:= true;
                  exit;
                end;
              dannado:= monstr.hp<(InfMon[monstr.TipoMonstruo].HPPromedio shr 1);
              if (distancia>5) and (monstr.mana>0) and not dannado then
              begin
                BuscarCaminoParaAvanzar(false,true);
              end
              else if dannado then
              begin
                P_DarVueltaDireccion(monstr.dir);
                BuscarCaminoParaAvanzar(true,true);
              end;
            end
            else
      ...
      begin
        ...
        if (abs(deltax)<=MaxRangoArqueroEnNormaCuadrado) and (abs(deltay)<MaxRangoArqueroEnNormaCuadrado) and//en rango
         ((banderas and bnInvisible=0) or longbool(monstr.banderas and bnVisionVerdadera)) then//visible
          begin
            monstr.dir:=MC_DarVueltaDireccion[calcularDireccion(deltax,deltay)];
            BuscarCaminoParaAvanzar(true,true);
            exit;
          end;
      end;
  ...
  function SinObjetivoDeAtaque(escapar,perseverar:boolean):boolean;
  ...
  begin
    ...
    with victima do
      ...
      begin
        deltax:=coordx-monstr.coordx;
        deltaY:=coordy-monstr.coordy;
        if (escapar and (hp>=monstr.hp)) then
        begin
          if (abs(deltax)<=MaxRangoSeguirEnNormaCuadrado) and (abs(deltay)<=MaxRangoSeguirEnNormaCuadrado) then
          begin
            monstr.dir:=MC_DarVueltaDireccion[calcularDireccion(deltax,deltay)];
            BuscarCaminoParaAvanzar(true,true);
          end
        ...
        begin
          if (banderas and bnOcultarse)=0 then
            monstr.dir:=CalcularDireccion(deltax,deltay);
          if ((banderas and bnInvisible)=0) or longbool(monstr.banderas and bnVisionVerdadera) then//visible
          begin
            // Realizar hechizos de monstruo
            if ataqueMonstruo(monstr,victima,aaMagica,Ninguno) then exit;
            BuscarCaminoParaAvanzar(false,true);
            exit;
          end;
        end
        else if (perseverar) then
        begin
          exit;
        end
        else if (abs(deltax)<=MaxRangoSeguirEnNormaCuadrado) and (abs(deltay)<=MaxRangoSeguirEnNormaCuadrado) then
        begin
          monstr.dir:=MC_DarVueltaDireccion[calcularDireccion(deltax,deltay)];
          BuscarCaminoParaAvanzar(true,true);
          exit;
  ...
  //Sólo para monstruos con ataque sólo melee
  //false si el monstruo está siguiendo a otro, true si no tiene un objetivo
  var deltax,deltay,rango:integer;
      casilla:word;
  begin
    ...
    begin
      deltax:=coordx-monstr.coordx;
      deltay:=coordy-monstr.coordy;
      rango:=MaxRangoSeguirEnNormaCuadrado shl 1;
      if activo and (abs(deltax)<=rango) and (abs(deltay)<=rango) then
      begin
    ...
    if codigo_de_bolsa<=MaxBolsas then
      with bolsa[codigo_de_bolsa] do
        if (tipo>=tbFogata) then//Fogatas:
          ...
          if (tipo=tbTrampaMagica) and (monstr.comportamiento<>comComerciante) then
            CaerEnLaTrampa;
  end;
...
begin
  ...
  ControlDeObjetosDelPiso;
  // Si es =0 el monstruo congelado nunca atacará.
  if longbool(monstr.banderas and bnCongelado) and ((Conta_Universal+monstr.codigo) and $3=3) then
    ContinuarElTickDeVida:=false;
      ...
      begin
        ...
        if victima<>nil then
          ...
          if not lugarVacioVerificarFronterasXY(monstr,posAFM_x,posAFM_y) then
            ...
            if SinObjetivoDeAtaque(true,false) then
              if random(6)=0 then
              begin
                case random(3) of
      ...
      begin
        ...
        if victima<>nil then
          ...
          if not lugarVacioVerificarFronterasXY(monstr,posAFM_x,posAFM_y) then
            ...
            if SinObjetivoDeAtaque(false,false) then
              if random(6)=0 then
              begin
                case random(3) of
                  0:P_anteriorDireccion(monstr.dir);
      ...
      begin
        ...
        if victima<>nil then//Atacar al enemigo
          ...
          if not lugarVacioVerificarFronterasXY(monstr,posAFM_x,posAFM_y) then
            ...
            if SinObjetivoDeAtaque(false,true) then
      ...
      comDefensaEstatica:begin
        ...
        begin
          if (monstr.banderas and bnMana)<>0 then
            if monstr.mana<250 then inc(monstr.mana,4);
            ...
            if (conta_Universal+codigo) and $3=0 then
              case random(16) of
                ...
                7:if abs(coordx-coordx_ant)+abs(coordy-coordy_ant)>=MAXIMA_DISTANCIA_COMERCIO then
                    self.mover(monstr,calcularDireccion(coordx_ant-coordx,coordy_ant-coordy),true)
                  else
...
//  assert(RJugador<>nil,'RJugador=nil en MuerteJugador');
  ...
  begin
    ...
    if ((BanderasMapa and bmEsMapaSeguro)=0) and (comportamiento<comModerador) then
  ...
  begin
    ...
    begin
      ...
      if modificador=0 then
        case MaximaCantidadDeEsteObjeto(Artefacto) of
          toCantidad_1: modificador:=random(50)+14;
          else modificador:=random(6)+1;
        end;
      ...
      id:=InfMon[Tipomonstruo].tesoro2;
      if id>=4 then
      begin
        modificador:=InfMon[Tipomonstruo].ModificadorTesoro2;
        if modificador=0 then
          case MaximaCantidadDeEsteObjeto(Artefacto) of
            toCantidad_1: modificador:=random(50)+14;
            else modificador:=random(6)+1;
      ...
      case id of
        ...
        4://Gemas talladas
        begin
          if InfMon[Tipomonstruo].nivelMonstruo>15 then
        ...
        5://Pociones
        begin
          id:=orPocimaInicial+random(random(9));//0..7
          modificador:=1+random(InfMon[Tipomonstruo].nivelMonstruo shr 2);
        end;
        6://oro
        begin
          id:=5;
          cantidad:=InfMon[Tipomonstruo].nivelMonstruo;
          modificador:=((cantidad+1) shr 1)+random(cantidad shr 2);
        end;
      end;
      if id>=4 then SoltarObjetoXY(Artefacto,coordx,coordy,TipoCadaver,true);
...
var    i:integer;
  ...
  begin
    ...
    begin
      ...
      if indiceDeMenorPrecio<=MAX_ARTEFACTOS then
...
begin
  ...
  with Bolsa[codBolsa] do
  begin
    if tipo<>tbninguna then// borrar marca de existencia de la bolsa reemplazada
    begin
...
begin
  ...
  begin
    ...
    PNJcomerciante.dir:=calcularDireccion(coordx-PNJcomerciante.coordx,coordy-PNJcomerciante.coordy);
  ...
  begin
    ...
    begin
      ...
      begin
        PNJcomerciante.inicializarTimer(TdBasico,TIEMPO_ATENCION_COMERCIANTE_AL_COMPRADOR);
        for i:=0 to MAX_ARTEFACTOS do
          if (item[i].id=ObjetoCantidadAVender.id) or (Tipo=TIPO_COMERCIANTE_MERCADER)then
          begin
...
begin
  with RJugador do
  begin
    if (not activo) or longbool(banderas and BnParalisis) or
      ((hp=0) and (comportamiento<=comHeroe)) then exit;
    x:= coordx;
    y:= coordy;
  end;
  ...
  begin
    // Revisar adelante
    inc(x,MC_avanceX[RJugador.dir]);
    inc(y,MC_avanceY[RJugador.dir]);
      ...
      begin
        for j:=0 to MAX_ARTEFACTOS do
          ...
          begin
            ...
            begin
              SendText(RJugador.codigo,'i'+char(i_TuInventarioEstaLleno));
  ...
  begin
    if (CodBolsa<=maxBolsas) then
      EnviarAlMapa_J(RJugador,#192{Eliminar bolso}+char(x)+char(y));
    SendText(RJugador.codigo,#193{Eliminar bolso vacio}+char(x)+char(y));
  end;
end;
    ...
    begin
      ...
      begin
        ...
        //Agregar control de condiciones de terreno.
          ihTijeras:if recurso=irSastreria then
...
procedure TTableroControlado.PrenderFogata(x,y:byte;codigoJugador:word);
  procedure ActualizarBanderasFogata(px,py:smallint);
  var i:integer;
      codMon:word;
  begin
    for i:=0 to MAX_INTENTOS_POSICIONAMIENTO-1 do //buscar jugadores
    begin
      codMon:=getMonRecXY(px+MC_POSICIONAMIENTO_X[i],py+MC_POSICIONAMIENTO_Y[i]);
      if (codMon and fl_con) = ccJgdr then
        with Jugador[codMon and fl_cod] do
          aurasExternas:= aurasExternas or flAuraExtFogata;
    end;
  end;
...
var
  Codigo_Bolsa:word;
begin
  // Obtener el código de la bolsa
  Codigo_Bolsa:=mapaPos[x,y].terbol and mskBolsa;
  if Codigo_Bolsa>maxBolsas then exit;
  // Existe una bolsa
  with Bolsa[Codigo_Bolsa] do
  begin
    if tipo<>tbLenna then exit;
    // El tipo de bolsa es leña lista para la fogata

    if EstaLloviendo(x,y) then
    begin
      if (codigoJugador<=MaxJugadores) then
        SendText(codigoJugador,'i'+char(i_NoPuedesEncenderFogataPorLluvia));
      exit;
    end;
...
var RecursoDelMapa:word;
  ...
  begin
    with RJugador do
    begin
      base:=(SAB+DES+random(25)) shr 4;
      if comida>0 then dec(comida);
      ...
      begin
        ...
        if random(20)=0 then
          if random(6)=0 then
          begin
            Arecurso.id:=98+random(1+random(6));//anillo
            Arecurso.modificador:=4+random(60);
          end
          else
  ...
  begin
    ...
    begin
      cantidad:=FRZ+DES;
      ConsumoComida:=1;
      if longbool(pericias and hbCarpinteria) then ConsumoComida:=0;
      if (ConsumoComida=0) then inc(cantidad,nivel);
      cantidad:=random(cantidad) shr 3;

      if ((cantidad<1) and (IDRecurso<>irLenna)) then
      begin
        if (random(4)>0) then exit;
        inc(ConsumoComida,2);
      end;
  ...
  begin
    ...
    with RJugador do
    begin
      base:=SAB+DES+irIngrediente0-IDRecurso;
      ...
      begin
        ...
        inc(ConsumoComida,IDRecurso-irIngrediente1);
        if comida<ConsumoComida then comida:=0 else dec(comida,consumoComida);
        SendText(codigo,#253+char(comida));
      end;
  ...
  begin
    ...
    begin
      ...
      base:= FRZ;
      if bono then inc(base,nivel);
      //3..120
      cantidad:=random(base);
      ...
      case IDRecurso of
        ...
        begin//Amatistas, topacios y aguamarinas.
          ...
          dec(cantidad,14+((ID-192) shl 1));
          ConsumoComida:=3;
        end;
        irGema0..irGema7:begin
          Arecurso.id:=192+IDRecurso-irGema0;
          dec(cantidad,20+((IDRecurso-irGema0) shl 1));
          ConsumoComida:=3;
        end;
        else
      ...
      if (not bono) then cantidad:=1;
      if (cantidad>25) then cantidad:=25;
  ...
  var log2cantidad,cantidad,lingotes:integer;
      recurso:byte;
  begin
    with RJugador do
    ...
    begin
      ...
      if (recurso=orHierro) or (recurso=orArcanita) then
        ...
        if (recurso=orPlata) then
          ...
          log2cantidad:=4;
      cantidad:=1 shl log2cantidad;
      lingotes:=Artefacto[IndArt].modificador;
      if lingotes<cantidad then exit;
      Artefacto[IndArt].id:=recurso+4;
      Artefacto[IndArt].modificador:=(lingotes+random(cantidad)) shr log2cantidad;
  ...
  begin
    with RJugador do
    if (maxMana>0) and (mana>=maxMana) then
    begin
      Nro_Conjuro:=Artefacto[IndArt].modificador;
      if puedeLeerElConjuro(Nro_Conjuro) then
      begin
        if (comida>25) then dec(comida,25) else comida:=0;
        mana:=0;
        Dificultar_Leer:=0;
        ...
        SendText(codigo,#253+char(comida)+#254+char(mana)+'Ic'+char(Nro_Conjuro));
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if Usando[uArmaIzq].modificador>1 then
        ...
        Usando[uArmaIzq]:=ObManoIzqVacia;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Arecurso.modificador:=1;
        Arecurso.id:=orGemaArruinada;
      end
      else
      begin
  ...
  begin
    ...
    begin
      ...
      case Artefacto[IndArt].id of
        orFlauta:begin
          TipoSonido:=#134;
          TiempoEfecto:=DES+nivel+16;
        end;
        orLaud:begin
          TipoSonido:=#135;
          TiempoEfecto:=((DES+nivel) shl 1)+16;
        end;
        else begin//orCuerno
          TipoSonido:=#136;
          TiempoEfecto:=(DES shl 1+CON+FRZ)+16;
        end
      end;
      if (TiempoEfecto)>255 then TiempoEfecto:=255;
  ...
  begin
    ...
    begin
      ...
      NotificarModificacionExperiencia(RJugador,modificador*1000);
      Artefacto[IndArt]:=ObNulo;
      SendText(codigo,char(216+IndArt)+char(id)+char(modificador));
    end;
  end;
begin
  ...
  with RJugador do
  if ((hp>0) or (artefacto[indArt].id=orBaulMagico))
    and ((banderas and BnParalisis)=0) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ClanAmigo>maxClanesJugadores then ClanAmigo:=ninguno;
      end;
    end;
    DuennoAmigo:=duenno;
    px:=coordx;
      ...
      begin
        ...
        begin
          ...
          case codigoM and fl_con of //case contenido de la casilla
            ...
            ccJgdr:begin
              with jugador[codigoM] do //sólo para jugadores, usar directamente el código de casilla como código de jugador
                if (clanAmigo<>clan) and (clan<>ninguno) and (hp<>0) then
                begin
                  monstruo_mirando.objetivoAtacado:=codigoM;
                  result:=MC_DE_Direcciones[i];
                  exit
              ...
              with Monstruo[codigoM and fl_cod] do
                ...
                //encantamiento
                  ...
                  begin
                    ...
                    monstruo_mirando.dir:=calcularDireccion(coordx-monstruo_mirando.coordx,coordy-monstruo_mirando.coordy);
...
begin
  with Rjugador do
  if (hp<>0) and (banderas and bnParalisis=0) and (comida>0) and
    (mana<maxMana) and (accion<>aaMeditando) then
    ...
    begin
      ...
      begin
        ...
        case tipo of
          tsResurreccion:
            with RJugador do
              ...
              begin
                ...
                RealizarYNotificarResureccionAvatar(Rjugador);
              end;
          ...
          tsFundarClan:
          begin
            if (pericias and hbElocuencia)<>0 then
...
begin
  ...
  with Rjugador,sensor[indiceDelSensor] do
    case llave1 of
      ...
      begin
        if (indiceDeLaLlave=0) then
          objVacio:=ObManoDerVacia
        else if (indiceDeLaLlave=1) then
          objVacio:=ObManoIzqVacia
        else
          objVacio:=ObNulo;

        usando[indiceDeLaLlave]:=objVacio;
    ...
    if RJugador.PuedeRecibirComando(8) then
      with sensor[indiceSensor] do
        ...
        begin
          ...
          begin
            distancia:=sqr(RJugador.coordx-x)+sqr(RJugador.coordy-y);
            if (distancia<=8) and (
              ((flagsSensor and fs_AreaActivaGrande)<>0) or //Si "mayor zona de efecto" o
              ((distancia<=5) and (RJugador.coordy>=(y-1)))) then//"dentro de menor zona"
            begin
