...
uses
  Forms,Graphics,Windows,TrayIcon,ScktComp,Menus,StdCtrls,Classes,Controls,
  globales,ComCtrls,demonios,tablero, ExtCtrls,Messages,GTimer;
const
  BASE_FPS_x_1000 = 7500;
...
type
  TMainForm = class(TForm)
    ...
    procedure Limpiarbolsas1Click(Sender: TObject);
...
uses Sysutils,usuarios,mundo,dialogs,objetos;
...
begin
  ...
  caption:='Artes Arcanas - Servidor '+getVersionCliente();
...
procedure TMainForm.ProcesarMensajesAplicacion(var Msg: TMsg; var Handled: Boolean);
begin
  if msg.message <> WM_GTIMER then exit;
...
begin
  if key=123 then
  begin
    Memo.lines.add('F12: Opciones de debug de aplicación activadas');
  ...
  begin
    ...
    if sender=Comunicacintotalcon1 then
      ...
      if TMenuItem(sender).Checked then
        Mensaje('ADVERTENCIA: En modo de Pruebas el servidor NO guardará los cambios realizados en objetos, dinero, experiencia, niveles, castillos, clanes, etc.'+
...
begin
  ...
  if Sizeof(TPersonaje) <> TJugadorS.InstanceSize - 4 then
    mensaje('(!) sizeof(TPersonaje) <> TJugadorS.InstanceSize - 4');
  ...
  if TAMANNO_DE_INSTANCIA_DE_TCLANJUGADORES <> TClanJugadores.InstanceSize then
    mensaje('(!) sizeof(TClanJugadores) <> TAMANNO_DE_INSTANCIA_DE_TCLANJUGADORES');
...
begin
  ...
  if sender=AgregarAdmin1 then
  begin
    EstadoNuevo:=euAdmin;
    nomCategoria:='Administrador';
  end
  else
  begin
    EstadoNuevo:=euModerador;
    nomCategoria:='Moderador';
  end;
...
begin
  ...
  begin
    ...
    if ID_Conexion<=MaxJugadores then//logueado
    begin
      if DatosUsuario[ID_Conexion].EstadoUsuario<euModerador then
      begin
...
procedure TMainForm.Limpiarbolsas1Click(Sender: TObject);
begin
  EliminarBolsas(nil);
end;
...
begin
  ...
  repeat
    ...
    if (nro<1) or (nro>1000) then
    begin
      showmessage('Ingresa una cantidad entre 1 y 1000.');
      continue;
    end
    else
...
begin
  ...
  if codJugador<=MaxJugadores then
    ...
    begin
      ...
      DatosUsuario[codJugador].TimerDesconeccionPorOcio:=TIEMPO_ANTES_DE_DESCONECTAR;
...
//Para login y Password:
  function GetTPassword:TSha256Hash;
  var i:integer;
  begin
    // BufferRecepcion is an string and therefore 1 based array
    inc(posicionBufferRecepcion);
    for i:=0 to 31 do
      result[i]:=ord(Socket.BufferRecepcion[posicionBufferRecepcion+i]);
    inc(posicionBufferRecepcion,31);
  end;
...
//Consumiendo Objetos:
  ...
  begin
    ...
    begin
      ...
      if result then
      begin
        error:=PuedeConsumir(IndArt);
        if (error=i_OK) or (error=i_EstaCrudo) then
...
//Usando Objetos:
  ...
  begin
    ...
    CadenaMensaje:=Get_Cadena127(nroBytes);
    result:=true;//todos los datos fueron recibidos
    if (DatosUsuario[codJugador].EstadoUsuario>=euModerador) then
      puedeHablar:=true
    else
      if (DatosUsuario[codJugador].AgresividadVerbal>=100) then
      begin
  ...
  var Login:TcadenaLogin;
      Password:TSha256Hash;
      nroBytes:byte;
  begin
    result:=false;
    if FaltaInformacion(33) then exit;
    Password:=getTPassword;//32bytes
      ...
      begin
        ...
        begin
          ...
          socket.SendTextNow('@'+
            ...
            char($2 or byte(ServidorEnModoDeVerificacion))+
  ...
  var i:integer;
  begin
    if longbool(DatosUsuario[codJugador].PermisosDelUsuario and puCrearObjeto) then
    with Jugador[codJugador] do
    begin
        ...
        case idObj of
            //Incrementa el nivel
            0:for i:=Jugador[codJugador].nivel to modObj do NotificarModificacionExperiencia(Jugador[codJugador],MAXIMA_EXPERIENCIA_FALTANTE);
  ...
  type TTipoMensajeBusqueda=(tmbNinguno,tmbBuscar,tmbRestaurar,tmbPrision,tmbExpulsar,tmbVisitar,tmbConvocar,tmbAnularClan,tmbPermitirChat,tmbNegarChat);
  ...
  begin
    if DatosUsuario[codJugador].EstadoUsuario<euModerador then exit;
    if IdMonstruo<=maxMonstruos then
      with monstruo[idMonstruo] do
        if (hp<>0) then
    ...
    begin
      with Jugador[codJugador] do
        if clan<=maxClanesJugadores then//tiene clan
          ...
          else if (Usando[0].id = 4) and (Usando[0].modificador = 30) then
          //es el lider, anula su clan
            with ClanJugadores[Jugador[codJugador].clan] do
            ...
            SendText(codJugador,'i'+char(i_30monedasDePlata));
    end
    else
      if Jugador[codJugador].clan<=maxClanesJugadores then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if faltaInformacion(32) then exit;
        Password:=getTPassword;//32bytes
      end;
      result:=true;
...
begin
  ...
  if DatosUsuario[codJugador].EstadoUsuario=euNoAutentificado then
    ...
    if HandshakeCorrecto(get4b,DatosUsuario[codJugador].UltimoIP) then
    begin
  ...
  begin
    ...
    case chr(GET1B) of
      ...
      'K':begin
        ...
        case char(get1b) of//Administrar Clan y Castillo
          ...
          'i':begin
            if FaltaInformacion(2) then exit;
            IntercambiarObjetoEnBaul(Jugador[codJugador],get1b{indice1},get1b{indice2});
          end;
          'S':begin
            if FaltaInformacion(2) then exit;
      ...
      'i':ActivarBerserker(Jugador[codJugador]);//Activar Berserker
      ...
      '&':begin//comando de GM
        if FaltaInformacion(1) then exit;
        case char(get1b) of
          'J':if (DatosUsuario[codJugador].EstadoUsuario>=euModerador)
                or ServidorEnModoDeComunicacionTotal then
                  ObtenerListaActivos(Jugador[codJugador],true{Listar jugadores});
          'K':if (DatosUsuario[codJugador].EstadoUsuario>=euModerador)
                or ServidorEnModoDeComunicacionTotal then
          ...
          'L':if longbool(DatosUsuario[codJugador].PermisosDelUsuario and puLimpiarMapas) then
            EliminarCadaveres(Jugador[codJugador]);
          'Y':if longbool(DatosUsuario[codJugador].PermisosDelUsuario and puEliminarBolsas) then
            EliminarBolsas(Jugador[codJugador]);
      ...
      'X':begin//comandos eXtendidos
        ...
        case char(get1b) of
          ...
          'E':begin//cambiar especialidad
            if FaltaInformacion(1) then exit;
            Jugador[codJugador].EspecialidadArma:=
             (Jugador[codJugador].EspecialidadArma and mskEspecialidadObjeto) or
             (get1b and mskEspecialidadFija);
          end;
          'C'://Cambiar contraseña
          begin
            if FaltaInformacion(32) then exit;//getTpassword
    ...
    begin
      ...
      if MensajesEnviadosEn16Turnos>128 then
      begin
...
procedure TMainForm.Limpiarcadveres1Click(
  Sender: TObject);
begin
