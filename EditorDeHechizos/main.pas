...
var f:textFile;
   ...
   begin
     if bytebool(Banderas and cjPuedeLanzarObjetivo) then result:='Preciso'
       ...
       if bytebool(Banderas and cjPuedeLanzarAsimismo) then result:='Zona'
       else
         result:='Lineal';
   end;
begin
...
begin
  EdtCosto.text:=inttostr(valor(EdtNivel.text,1,25) +
    ((valor(EdtINT.text,1,20) + valor(EdtSAB.text,1,20)) shr 1) +
    valor(EdtMana.text,1,20));
end;
