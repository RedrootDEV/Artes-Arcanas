...
function NivelDannoWeb(base,bono:integer):string;
begin
  if bono<=1 then
    if base=0 then
      result:='---'
...
//artefacto o menor.
  ...
  begin
    ...
    begin
      ...
      if (i shr 3)=17 then cantidad:=CantidadConstruida;
      if cantidad>1 then total:=total div cantidad;
...
begin
  ...
  begin
    ...
    if codigoAnterior shr 3=1 then
      LB_Modificador.Caption:='Bono por a.mágica [40..80]'
    else
  ...
  begin//no son armas
    ...
    begin
      LB_Modificador.Caption:='Penalización/Bono [-30..30] ';
      GB_armadura.show;
    ...
    begin
      LB_Modificador.Caption:='Modificador [0..110] ';
      GB_armadura.hide;
      e_punz.text:='0';
      e_cort.text:='0';
      e_golp.text:='0';
  ...
  begin
    temp:=des.recursoX[2] and $7;
    if (temp=0) and (des.cantidadX[2]>0) then temp:=8;
    E_recurso3.text:=inttostr(temp);
  end
  else
...
begin
  if CB_clase.ItemIndex=1 then
    E_ModADC.text:=controlar(E_ModADC.text,40,80)
  else
    if CB_clase.ItemIndex<=6 then
      E_ModADC.text:=controlar(E_ModADC.text,-25,25)
    else
      if CB_clase.ItemIndex<=12 then
        E_ModADC.text:=controlar(E_ModADC.text,-30,30)
      else
        E_ModADC.text:=controlar(E_ModADC.text,0,110);
end;
...
begin
  ...
  rewrite(f);
  writeln(f,'<center><h1>Armas</h1><table border=1 cellpadding=5 cellspacing=0>');
  writeln(f,'<tr><th colspan=2>Nombre</th><th>Daño<BR>PM</th><th>Daño<BR>G</th><th>Tipo<BR>de daño</th><th>Requisito</th>'+
  {'<th>Clases</th><th>Razas</th>}'<th>Ataque</th><th>Nivel<BR>mínimo</th></tr>');
  for i:=0 to 55 do
  with Info,datos[i] do
  begin
    if (i<2) or (i>=16) then
    ...
    begin
      writeln(f,'<tr><td width=30 height=40 style="background-image:url(../laa/grf/obj.jpg);background-position:-'+
        intastr((i and $7)*40)+'px -'+intastr((i shr 3)*40)+'px;"></td><td>'+nombreArma(i)+
      '</td><td>'+NivelDannoWeb(danno1b,danno1p)+
      '</td><td>'+NivelDannoWeb(danno2b,danno2p)+'</td><td>'+cb_Tipo.Items[ord(TipoArma)]+
...
begin
  ...
  rewrite(f);
  writeln(f,'<center><h1>Armas Conjurables</h1><table border=1 cellpadding=5 cellspacing=0>');
  ...
  begin
    ...
    begin
      writeln(f,'<tr><td width=30 height=40 style="background-image:url(../laa/grf/obj.jpg);background-position:-'+
        intastr((i and $7)*40)+'px -'+intastr((i shr 3)*40)+'px;"></td><td>'+info.nombre[i]+'</td><td>'+info.nombre[i+orGemaInicial-8]+
      '</td><td>'+NivelDannoWeb(danno1b,danno1p)+
      '</td><td>'+NivelDannoWeb(danno2b,danno2p)+'</td><td>'+cb_Tipo.Items[ord(TipoArma)]+
      '</td><td>'+ModAtaque(modificadorADC)+'</td><td>'+MostrarNivel(nivelminimo)+'</td></tr>');
    end;
  end;
  writeln(f,'</table></center>');
  closefile(f);
...
begin
  ...
  rewrite(f);
  writeln(f,'<center><h1>Armaduras y Vestimentas</h1><table border=1 cellpadding=5 cellspacing=0 style="table-layout:fixed;">');
  writeln(f,'<tr><th colspan=2 rowspan=2>Nombre</th><th colspan=4>Reducción de daño</th><th rowspan=2>Evasión</th><th rowspan=2>Nivel<BR>mínimo</th></tr>');
